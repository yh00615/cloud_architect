...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({
            ...
            if (match[2]) {
                ...
                parts.push(
                    <span key={match.index} className="markdown-button-wrapper">
                        <AWSButton variant="primary" size="small">{match[2]}</AWSButton>
                    </span>
                )
            } else if (match[3]) {
                // {{버튼}} - Normal
                parts.push(
                    <span key={match.index} className="markdown-button-wrapper">
                        <AWSButton variant="normal" size="small">{match[3]}</AWSButton>
                    </span>
                )
            } else if (match[4]) {
                // ((버튼)) - Link
                parts.push(
                    <span key={match.index} className="markdown-button-wrapper">
                        <AWSButton variant="link" size="small">{match[4]}</AWSButton>
                    </span>
                )
            }
    ...
    const components = {
        ...
        blockquote: ({ children }: any) => {
            ...
            const renderDownloadBlock = (children: React.ReactNode): React.ReactNode => {
                            ...
                            if (Array.isArray(pChildren)) {
                                for (const pChild of pChildren) {
                                    ...
                                    if (pChild?.props?.node?.tagName === 'a' && pChild?.props) {
                                        ...
                                        downloadUrl = pChild.props.href || pChild.props.node?.properties?.href
                                        break
                                    }
                                }
                            } else if (pChildren?.props?.node?.tagName === 'a' && pChildren?.props) {
                                fileName = extractText(pChildren.props.children)
                                downloadUrl = pChildren.props.href || pChildren.props.node?.properties?.href
                            }
                        }

                        // ul 태그 확인 (파일 목록 또는 관련 태스크 추출)
                        if (child?.props?.node?.tagName === 'ul' && child?.props?.children) {
                            const ulChildren = child.props.children

                            if (Array.isArray(ulChildren)) {
                                ulChildren.forEach((liChild) => {
                                    // li 태그 확인
                                    if (liChild?.props?.node?.tagName === 'li' && liChild?.props?.children) {
            ...
            const formatAlertContent = () => {
                ...
                const processNode = (node: any): void => {
                    ...
                    if (typeof node === 'string') {
                        // Alert 마커 제거
                        const cleaned = node.replace(/\[!(NOTE|WARNING|TIP|ERROR|SUCCESS|COST|IMPORTANT|TROUBLESHOOTING|CONCEPT)\]/g, '').trim()
                        if (cleaned) {
                            result.push(
                    ...
                    // table 처리
                    if (node?.type === 'table' || node?.props?.node?.tagName === 'table') {
                        result.push(<table key={`table-${key++}`} className="markdown-table">{node.props?.children}</table>)
                        return
                    }

                    // ul/ol 리스트 처리
                    if (node?.type === 'ul' || node?.type === 'ol' ||
                        node?.props?.node?.tagName === 'ul' || node?.props?.node?.tagName === 'ol') {
                        const items = Array.isArray(node.props?.children) ? node.props.children : [node.props?.children]
                    ...
                    if (node?.type === 'p' || node?.props?.node?.tagName === 'p') {
                        const text = extractText(node.props?.children)
                        const cleaned = text.replace(/\[!(NOTE|WARNING|TIP|ERROR|SUCCESS|COST|IMPORTANT|TROUBLESHOOTING|CONCEPT)\]/g, '').trim()
                        if (cleaned) {
                            result.push(
                                <div key={`text-${key++}`} className="alert-content-text">
                                    {cleaned}
                    ...
                    // strong 태그 처리
                    if (node?.type === 'strong' || node?.props?.node?.tagName === 'strong') {
                        const text = extractText(node.props?.children)
                        result.push(
                ...
                const filteredChildren = Array.isArray(children)
                    ? children.filter((child: any) => {
                        if (typeof child === 'string') {
                            return !child.match(/\[!(NOTE|WARNING|TIP|ERROR|SUCCESS|COST|IMPORTANT|TROUBLESHOOTING|CONCEPT)\]/)
                        }
                        return true
                    })
                    : children
        ...
        a: ({ href, children }: any) => {
            ...
            if (href?.startsWith('aws:')) {
                ...
                return (
                    <AWSButton
                        href={`https://console.aws.amazon.com/${service}`}
                    >
                        {children}
                    </AWSButton>
                )
        ...
        p: ({ children }: any) => {
            ...
            for (const pattern of statusPatterns) {
                const regex = new RegExp(`^${pattern.emoji}\\s*\\*\\*${pattern.label}\\*\\*:?\\s*(.*)$`)
                    ...
                    return (
                        ...
                        >
                            <span className="markdown-status-wrapper">
                                <StatusIndicator type={pattern.type}>{pattern.label}</StatusIndicator>
            ...
            return (
                <Box
                    ...
                    margin={{ top: 'xs', bottom: 'xs' }}
                >
                    <span className="markdown-paragraph-text">
                        {finalChildren}
                    </span>
                </Box>
            )
        ...
        h1: ({ children }: any) => {
            ...
            return (
                <Box variant="h1" margin={{ top: 'l', bottom: 'm' }} id={id || undefined}>
                    <span className="markdown-h1">
                        {children}
                    </span>
                </Box>
            )
        ...
        h2: ({ children }: any) => {
            ...
            // 구분선이 필요한 섹션들
            const needsDivider = !isReference && (
                text.includes('마무리') ||
            ...
            if (isTask || isDemo) {
                return (
                    <>
                        ...
                        <Box variant="h3" margin={{ top: 'm', bottom: 's' }} id={id || undefined}>
                            <span className="markdown-h2">
                                {children}
                            </span>
                        </Box>
                    </>
            ...
            if (needsDivider) {
                return (
                    <>
                        ...
                        <Box variant="h2" margin={{ top: 'm', bottom: 's' }} id={id || undefined}>
                            <span className="markdown-h2">
                                {children}
                            </span>
                        </Box>
            ...
            // 일반 h2 (구분선 없음)
            return (
                <Box variant="h2" margin={{ top: 'l', bottom: 's' }} id={id || undefined}>
                    <span className="markdown-h2">
                        {children}
                    </span>
                </Box>
