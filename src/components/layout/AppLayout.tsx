...
export const AppLayout: React.FC<AppLayoutProps> = ({
            ...
            .map(week => {
                ...
                return {
                    ...
                    items: visibleSessions.map(session => {
                        // 타입별 아이콘 - Dashboard와 완전히 동일하게
                        const getTypeIcon = () => {
                            if (session.type === 'lab') return '✓';     // status-positive 아이콘과 동일
                            if (session.type === 'demo') return '▶';    // video-on 아이콘과 동일
                            return '📄';                                 // file 아이콘과 동일
                        };

                        return {
                            type: "link" as const,
                            text: `${getTypeIcon()} ${week.week}-${session.session}. ${session.title}`,
    ...
    const helpPanel = (
        <HelpPanel
            header={<Box variant="h3">AWS 용어 사전</Box>}
            footer={null}
        >
            <HelpPanelContent />
        </HelpPanel>
