...
export const SessionGuide: React.FC = () => {
    const { weekNumber, sessionNumber } = useParams<{ weekNumber: string; sessionNumber: string }>()
        ...
        <SpaceBetween direction="vertical" size="l">
            {[
                    ...
                    {metadata?.awsServices && metadata.awsServices.length > 0 && (
                        <Box margin={{ top: 's' }}>
                            ...
                            <SpaceBetween direction="horizontal" size="xs">
                                {Array.isArray(metadata.awsServices) && metadata.awsServices.map((service: string, index: number) => (
                                    <span
                                        key={`service-${index}`}
                ...
                <Container
                    ...
                    header={
                        <Header variant="h2">
                            <span className="session-section-title main-header">📋 {sessionData.type === 'lab' ? '실습' : '데모'} 개요</span>
                        </Header>
                    }
                >
                    <SpaceBetween direction="vertical" size="m">
                        {/* 실습/데모 목표 및 주요 태스크 */}
                        <ColumnLayout key="learning-objectives" columns={2} variant="text-grid">
                            <div key="learning-objectives-section">
                                <Box variant="h2" padding={{ bottom: 's' }}>
                                    <span className="session-section-title sub-header">🎯 {sessionData.type === 'lab' ? '실습' : '데모'} 목표</span>
                                </Box>
                                {metadata?.learningObjectives && metadata.learningObjectives.length > 0 ? (
                                    <ul className="session-objectives-list">
                                        {metadata.learningObjectives.map((objective: string, index: number) => (
                            ...
                            <div key="tasks-section">
                                <Box variant="h2" padding={{ bottom: 's' }}>
                                    <span className="session-section-title sub-header">📋 주요 태스크</span>
                                </Box>
                                {markdownContent && extractTasks(markdownContent).length > 0 ? (
                                    <ul className="session-tasks-list">
                                        {extractTasks(markdownContent).map((task: string, index: number) => (
                                            <li key={`task-${index}`}>
                                                {task}
                                            </li>
                                        ))}
                        ...
                        {/* 실습 정보 카드 */}
                        {(metadata?.resources || metadata?.estimatedCost || metadata?.freeTier) && (
                                                ...
                                                <SpaceBetween direction="vertical" size="xs">
                                                    ...
                                                    <Box color="text-body-secondary">
                                                        <SpaceBetween direction="vertical" size="xxs">
                                                            {metadata.resources.map((resource: string, index: number) => (
                                                                <div key={`resource-${index}`}>• {resource}</div>
                                                            ))}
                                                        </SpaceBetween>
                                                    </Box>
                                                </SpaceBetween>
                                            ...
                                            <Box key="free-tier">
                                                <SpaceBetween direction="vertical" size="xs">
                                                    <Box>
                                                        <SpaceBetween direction="horizontal" size="xs" alignItems="center">
                                                            <Icon
                                                                name={metadata.freeTier ? "status-positive" : "status-negative"}
                                                                variant={metadata.freeTier ? "success" : "error"}
                                                            />
                        ...
                        {metadata?.downloadFiles && metadata.downloadFiles.length > 0 && (
                            <div key="download-files" className="download-files-section">
                                ...
                                <ColumnLayout
                                    ...
                                    className={metadata.downloadFiles.length === 1 ? 'download-files-grid-single' : ''}
                                >
                                    {metadata.downloadFiles.map((file: any, index: number) => (
                                        <div key={index} className="download-file-card">
                                            <div className="download-file-header">
                                                    ...
                                                    content={
                                                        <Box padding="m">
                                                            <SpaceBetween direction="vertical" size="m">
                                                                ...
                                                                <hr key="divider" className="download-file-divider" />
                                                                ...
                                                                <div key="resources">
                                                                    ...
                                                                    <Box>
                                                                        <SpaceBetween direction="vertical" size="xxs">
                                                                            <div key="resource-1">• VPC 및 서브넷 (퍼블릭/프라이빗)</div>
                                                                            <div key="resource-2">• EC2 인스턴스 2개</div>
                                                                            <div key="resource-3">• 보안 그룹 및 IAM Role</div>
                                                ...
                                                >
                                                    <Button variant="icon" iconName="status-info" ariaLabel="파일 정보 보기" />
                                                </Popover>
                                            </div>
                                            <Button
                    ...
                    header={
                        <Header variant="h2">
                            <span className="guide-header-title">
                                {sessionData.type === 'demo' ? '🎥 데모 가이드' : '🎯 실습 가이드'}
                            </span>
                        </Header>
                    }
                >
                        ...
                        isLoading ? (
                            <Box textAlign="center" padding="xxl" >
                                <Box variant="p" color="text-body-secondary">
                                    {sessionData.type === 'demo' ? '데모 가이드를' : '실습 가이드를'} 불러오는 중...
                                </Box>
                            </Box>
                        ) : error ? (
                            ...
                            <Alert type="warning" header="콘텐츠 준비 중">
                                상세한 {sessionData.type === 'demo' ? '데모' : '실습'} 가이드는 곧 업데이트될 예정입니다.
                            </Alert>
                        )}
                </Container>,

                (metadata?.keyConcepts || metadata?.bestPractices || metadata?.warnings) && (
                    <Container
                        key="key-points-card"
                        header={
                            <Header variant="h2">
                    ...
                    >
                        <SpaceBetween direction="vertical" size="l">
                            ...
                            <Box padding={{ horizontal: 'l', vertical: 'm' }}>
                                <SpaceBetween direction="vertical" size="xs">
                                    {[
                                        ...(metadata.keyConcepts || []).map((item: string, idx: number) => ({
                                            icon: '💡',
                                            title: `핵심 ${idx + 1}`,
                                            content: item,
                                            type: 'concept'
                                        })),
                                        ...(metadata.bestPractices || []).map((item: string, idx: number) => ({
                                            icon: '✅',
                                            title: `실무 ${idx + 1}`,
                                            content: item,
                                            type: 'practice'
                                        })),
                                        ...(metadata.warnings || []).map((item: string, idx: number) => ({
                                            icon: '⚠️',
                                            title: `주의 ${idx + 1}`,
                                            content: item,
                                            type: 'warning'
