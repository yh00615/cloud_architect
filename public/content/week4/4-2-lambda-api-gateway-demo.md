...
> [!IMPORTANT]
> 이 출력값은 태스크 4.1에서 환경 변수로 사용됩니다. 반드시 메모장에 저장하세요.
...
1. **Self-service sign-up**에서 `Enable self-registration`을 체크합니다.
2. **Attribute verification and user account confirmation**은 기본값을 유지합니다.
3. **Required attributes**에서 `name`을 체크합니다.
4. [[Next]] 버튼을 클릭합니다.
...
5. **Initial app client** 섹션에서 다음을 설정합니다:
   - **App client name**에 `QuickTableAppClient`를 입력합니다.
   - **Client secret**에서 `Don't generate a client secret`를 선택합니다.
6. 아래로 스크롤하여 **Advanced app client settings** 섹션을 확장합니다.
7. **Authentication flows** 섹션에서 `ALLOW_USER_PASSWORD_AUTH`를 체크합니다.
...
8. 아래로 스크롤하여 **Tags - optional** 섹션을 확장합니다.
...
1. 왼쪽 메뉴에서 **Resources**를 선택합니다.
2. `/reservations` 리소스를 확장합니다.
3. `POST` 메서드를 선택합니다.
4. **Method Request** 섹션에서 [[Edit]] 버튼을 클릭합니다.
...
5. **Authorization**에서 `CognitoAuthorizer`를 선택합니다.
6. [[Save]] 버튼을 클릭합니다.

### 태스크 3.3: GET 메서드에 Authorizer 연결

1. `/reservations` 리소스에서 `GET` 메서드를 선택합니다.
2. **Method Request** 섹션에서 [[Edit]] 버튼을 클릭합니다.
...
3. **Authorization**에서 `CognitoAuthorizer`를 선택합니다.
4. [[Save]] 버튼을 클릭합니다.

### 태스크 3.4: API 재배포

1. [[Deploy API]] 버튼을 클릭합니다.
...
1. 다음 명령어로 로그인하여 인증 토큰을 획득하고 환경 변수에 저장합니다:
...
1. CloudShell에서 다음 명령어를 실행하여 예약을 생성합니다:
...
1. Authorization 헤더 없이 API를 호출합니다:
