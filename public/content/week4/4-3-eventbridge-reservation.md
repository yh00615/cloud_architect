...
이 실습에서는 QuickTable 레스토랑 예약 시스템에 EventBridge 기반 이벤트 처리를 추가합니다. 예약 생성 시 자동으로 테이블 재고를 확인하고, 예약 가능 여부에 따라 고객에게 알림을 발송하는 이벤트 기반 아키텍처를 구축합니다. Week 4-2에서 구축한 예약 API와 자연스럽게 연결되어 전체 예약 시스템을 완성합니다.

> [!DOWNLOAD]
> [week4-3-quicktable-events-lab.zip](/files/week4/week4-3-quicktable-events-lab.zip)
>
> - `week4-3-quicktable-events-lab.yaml` - AWS CloudFormation 템플릿 (태스크 0에서 Amazon DynamoDB 테이블, EventBridge Event Bus, AWS Lambda 함수 3개, AWS Lambda 역할, Amazon SNS Topic 자동 생성)
> - `reservation_processor.py` - 예약 생성 AWS Lambda 함수 코드 (참고용 - 태스크 0에서 CloudFormation이 자동 생성)
> - `table_availability_checker.py` - 테이블 재고 확인 AWS Lambda 함수 코드 (참고용 - 태스크 0에서 CloudFormation이 자동 생성)
> - `notification_sender.py` - 알림 발송 AWS Lambda 함수 코드 (참고용 - 태스크 0에서 CloudFormation이 자동 생성)
> - `README.md` - 이벤트 패턴 설명 및 AWS Lambda 함수 코드 상세 해설
>
> **관련 태스크:**
>
...
- **AWS Lambda 실행 역할**: Amazon DynamoDB, EventBridge, Amazon SNS 접근 권한
- **Amazon SNS Topic**: ReservationNotifications (알림 발송)
- **Amazon CloudWatch Logs Groups**: AWS Lambda 함수 로그 저장

> [!NOTE]
> EventBridge 규칙(Rules)은 CloudFormation에 포함되지 않습니다. 태스크 1-2에서 학생이 직접 이벤트 패턴을 정의하고 규칙을 생성합니다.
>
...
>
> **날짜 변경 방법**: 실습 시점이 2026-03-20 이후라면 CloudFormation 템플릿의 `InitializeRestaurantAvailability` 함수 코드에서 날짜를 수정할 수 있습니다. 템플릿 파일을 텍스트 에디터로 열어 `2026-03-20`을 원하는 날짜로 변경한 후 스택을 생성합니다. 태스크 3-4의 테스트 이벤트 JSON에서도 동일한 날짜를 사용해야 합니다.
>
...
> Amazon DynamoDB 테이블, EventBridge Event Bus, AWS Lambda 역할, Amazon SNS Topic이 순차적으로 생성됩니다.
...
> [!IMPORTANT]
> **출력값 메모**: 이후 태스크에서 사용되는 출력값은 `ReservationEventBusName` 하나뿐입니다. 태스크 1-2에서 EventBridge 규칙 생성 시 Event Bus를 선택할 때 참고합니다. 나머지 출력값(LambdaExecutionRoleArn, ReservationNotificationTopicArn 등)은 참고용이며 실습에서 직접 사용하지 않습니다.
>
> **날짜 변경 방법 (중요)**: 실습 시점이 2026-03-20 이후라면 다음 두 가지를 모두 수정해야 합니다:
>
> 1. **CloudFormation 템플릿**: `week4-3-quicktable-events-lab.yaml` 파일을 텍스트 에디터로 열어 `InitializeAvailabilityFunction` 함수 코드에서 `2026-03-20`을 원하는 날짜로 변경한 후 스택을 생성합니다.
...
이 태스크에서는 EventBridge에서 ReservationCreated 이벤트를 수신하여 TableAvailabilityChecker AWS Lambda 함수를 트리거하는 규칙을 생성합니다.

### 태스크 설명

EventBridge 규칙은 이벤트 패턴을 정의하여 특정 이벤트만 필터링하고 대상 서비스로 전달합니다. 이 태스크에서는 `source=reservation.service`, `detail-type=ReservationCreated` 조건을 만족하는 이벤트만 TableAvailabilityChecker AWS Lambda 함수로 라우팅하는 규칙을 생성합니다.

### 상세 단계

> [!IMPORTANT]
> **태스크 1과 태스크 2의 차이점**: 이 두 태스크는 EventBridge 규칙 생성 과정이 거의 동일하지만, 다음 3가지만 다릅니다:
>
> 1. **규칙 이름 (Name)**: ReservationCreatedRule vs TableUnavailableRule
> 2. **이벤트 패턴 JSON**: source와 detail-type 값이 다름
> 3. **대상 Lambda 함수**: TableAvailabilityChecker vs NotificationSender
>
> 나머지 단계는 모두 동일하므로 이 3가지 차이점에 집중하여 진행합니다.

1. AWS Management Console에 로그인한 후 상단 검색창에서 `EventBridge`를 검색하고 선택합니다.
...
4. [[Create rule]] 버튼을 클릭합니다.
5. **Define rule detail** 페이지에서 다음을 입력합니다:
...
6. **Rule type**에서 `Rule with an event pattern`을 선택합니다.
7. [[Next]] 버튼을 클릭합니다.
8. **Build event pattern** 페이지에서 **Event source**는 `Other`를 선택합니다.
9. **Creation method**에서 `Custom pattern (JSON editor)` 또는 `Edit pattern`을 선택합니다.

> [!NOTE]
> EventBridge 콘솔에서 이벤트 패턴을 입력하는 방법은 두 가지입니다:
>
...
10. **Event pattern** 섹션에서 다음 JSON을 입력합니다:
...
> 이벤트 패턴은 EventBridge가 이벤트를 필터링하는 규칙입니다. JSON 형식으로 정의하며, `source`, `detail-type`, `detail` 필드를 기준으로 매칭합니다.
>
...
11. [[Next]] 버튼을 클릭합니다.
12. **Select target(s)** 페이지에서 **Target types**는 `AWS service`를 선택합니다.
13. **Select a target**에서 `AWS Lambda function`을 선택합니다.
14. **Function** 드롭다운에서 `TableAvailabilityChecker`를 선택합니다.

> [!NOTE]
> TableAvailabilityChecker AWS Lambda 함수는 태스크 0에서 CloudFormation이 자동으로 생성했습니다. 함수 이름은 `week4-3-quicktable-events-lab-TableAvailabilityChecker` 형식입니다.

15. [[Next]] 버튼을 클릭합니다.
16. **Configure tags** 페이지에서 [[Add new tag]] 버튼을 클릭한 후 다음 태그를 추가합니다:
...
17. [[Next]] 버튼을 클릭합니다.
18. **Review and create** 페이지에서 설정을 확인합니다.
19. [[Create rule]] 버튼을 클릭합니다.

> [!NOTE]
> EventBridge 규칙이 AWS Lambda 함수를 트리거할 수 있도록 권한이 자동으로 추가됩니다.

규칙 생성 후 필요에 따라 수정하거나 삭제할 수 있습니다.

> [!TIP]
> **EventBridge 규칙 수정 및 삭제**: 생성한 규칙을 수정하거나 삭제하려면 EventBridge 콘솔의 Rules 페이지에서 규칙을 선택한 후 [[Edit]] 또는 [[Delete]] 버튼을 클릭합니다. 규칙 삭제 시 확인 창에 `delete`를 입력해야 합니다 (규칙 이름이 아님).

✅ **태스크 완료**: ReservationCreated 이벤트 규칙이 생성되었습니다.

## 태스크 2: TableUnavailable 이벤트 규칙 생성

이 태스크에서는 EventBridge에서 TableUnavailable 이벤트를 수신하여 NotificationSender AWS Lambda 함수를 트리거하는 규칙을 생성합니다.
...
12. **Select target(s)** 페이지에서 **Target types**는 `AWS service`를 선택합니다.
13. **Select a target**에서 `AWS Lambda function`을 선택합니다.
14. **Function** 드롭다운에서 `NotificationSender`를 선택합니다.
15. [[Next]] 버튼을 클릭합니다.
16. **Configure tags** 페이지에서 [[Add new tag]] 버튼을 클릭한 후 다음 태그를 추가합니다:
...
ReservationProcessor 함수를 테스트 이벤트로 실행하면 ReservationCreated 이벤트가 발행되고, 태스크 1에서 생성한 EventBridge 규칙이 이 이벤트를 감지하여 TableAvailabilityChecker 함수를 자동으로 트리거합니다.

**이벤트 흐름**:

1. ReservationProcessor → DynamoDB에 예약 저장
2. ReservationProcessor → EventBridge에 ReservationCreated 이벤트 발행
3. EventBridge → ReservationCreatedRule 규칙 매칭
4. EventBridge → TableAvailabilityChecker 함수 트리거
5. TableAvailabilityChecker → 가용성 확인
6. (불가 시) TableAvailabilityChecker → EventBridge에 TableUnavailable 이벤트 발행
7. (불가 시) EventBridge → TableUnavailableRule 규칙 매칭
8. (불가 시) EventBridge → NotificationSender 함수 트리거

> [!NOTE]
> 이 실습에서는 AWS Lambda 함수를 직접 테스트하여 이벤트 기반 워크플로우를 확인합니다. 실제 프로덕션 환경에서는 API Gateway를 통해 ReservationProcessor 함수를 호출하며, 사용자 요청이 자동으로 이벤트 기반 워크플로우를 트리거합니다.

### AWS Lambda 함수 코드 확인

ReservationProcessor AWS Lambda 함수는 태스크 0에서 CloudFormation이 자동으로 생성했습니다. 다음은 함수의 핵심 코드입니다 (참고용):

```python
# 예약 데이터를 DynamoDB에 저장
dynamodb.put_item(
...
2. `ReservationProcessor` 함수를 선택합니다.

> [!NOTE]
> ReservationProcessor AWS Lambda 함수는 태스크 0에서 CloudFormation이 자동으로 생성했습니다. 함수 이름은 `week4-3-quicktable-events-lab-ReservationProcessor` 형식입니다.

3. **Test** 탭을 선택합니다.
4. **Test event action**에서 `Create new event`를 선택합니다.
5. **Event name**에 `TestReservationAvailableEvent`를 입력합니다.
6. **Event JSON**에 다음 내용을 입력합니다:
...
> [!IMPORTANT]
> **날짜 변경 필수**: 실습 시점이 2026-03-20 이후라면 `date` 필드를 실습 시점 기준 미래 날짜로 변경합니다. 단, CloudFormation 템플릿의 초기 데이터 날짜도 동일하게 수정해야 합니다 (태스크 0 출력값 확인 섹션의 IMPORTANT Alert 참조).
>
...
8. [[Test]] 버튼을 클릭합니다.
9. 함수 실행이 완료됩니다.
10. **Execution result** 섹션에서 실행 결과를 확인합니다.
...
11. **Monitor** 탭을 선택합니다.
12. **View Amazon CloudWatch Logs** 링크를 클릭합니다.

> [!NOTE]
> Amazon CloudWatch Logs는 1-2분의 지연이 있을 수 있습니다. 로그가 표시되지 않으면 1-2분 대기 후 페이지를 새로고침합니다.

13. 최신 로그 스트림을 선택합니다.
14. 로그에서 "ReservationCreated event published" 메시지를 확인합니다.

> [!NOTE]
> 이 메시지는 ReservationProcessor 함수가 EventBridge에 이벤트를 성공적으로 발행했음을 의미합니다.

15. AWS Lambda 콘솔로 이동합니다.
16. 검색창에 `TableAvailabilityChecker`를 입력하여 함수를 찾습니다.
17. `TableAvailabilityChecker` 함수를 선택합니다.

> [!NOTE]
> 함수 이름이 `week4-3-quicktable-events-lab-TableAvailabilityChecker`로 길기 때문에 검색창을 사용하면 쉽게 찾을 수 있습니다.
>
> **EventBridge 전달 지연**: ReservationProcessor 실행 후 EventBridge가 TableAvailabilityChecker를 트리거하기까지 수 초~1분이 소요될 수 있습니다. 로그가 보이지 않으면 1-2분 대기 후 새로고침합니다.

18. **Monitor** 탭을 선택합니다.
19. **View Amazon CloudWatch Logs** 링크를 클릭합니다.

> [!NOTE]
> Amazon CloudWatch Logs는 1-2분의 지연이 있을 수 있습니다. 로그가 표시되지 않으면 1-2분 대기 후 페이지를 새로고침합니다.

CloudWatch Logs 콘솔에서 로그 스트림 목록이 표시됩니다.

> [!TIP]
> **로그 스트림 식별 방법**: CloudWatch Logs 콘솔에서 로그 스트림 목록이 표시되면 **Last event time** 컬럼을 기준으로 정렬하여 가장 최근 스트림을 선택합니다. 기본적으로 최신 순으로 정렬되어 있으므로 목록 맨 위의 스트림을 선택하면 됩니다.

20. 최신 로그 스트림을 선택합니다.
21. 로그에서 "Table available: party size (2) within available slots (5)" 메시지를 확인합니다.

> [!NOTE]
> TableAvailabilityChecker 함수가 자동으로 실행되었다면 태스크 1에서 생성한 EventBridge 규칙이 정상적으로 동작하는 것입니다. 예약 가능한 경우 TableUnavailable 이벤트가 발행되지 않으므로 NotificationSender 함수는 실행되지 않습니다.

만약 로그가 표시되지 않는다면 다음 문제 해결 방법을 시도합니다.

> [!TROUBLESHOOTING]
> **문제**: TableAvailabilityChecker 함수의 CloudWatch Logs가 생성되지 않습니다
>
> **증상**: Monitor 탭에서 "View Amazon CloudWatch Logs" 링크를 클릭해도 로그 스트림이 표시되지 않습니다.
>
> **원인**: EventBridge 규칙이 올바르게 설정되지 않았거나, Lambda 함수 권한이 부족합니다.
>
> **해결**:
>
> 1. EventBridge 콘솔에서 ReservationCreatedRule 규칙의 Targets 섹션에 TableAvailabilityChecker 함수가 연결되어 있는지 확인합니다.
> 2. Lambda 콘솔에서 TableAvailabilityChecker 함수의 Configuration > Permissions > Resource-based policy statements에서 EventBridge 호출 권한을 확인합니다.
> 3. ReservationProcessor 함수로 돌아가 테스트 이벤트를 다시 실행한 후 1-2분 대기합니다.
> 4. ReservationProcessor 함수로 돌아가 테스트 이벤트를 다시 실행한 후 1-2분 대기합니다.

### EventBridge 규칙 동작 확인

21. EventBridge 콘솔로 이동합니다.
22. 왼쪽 메뉴에서 **Rules**를 선택합니다.
23. **Event bus** 드롭다운에서 `week4-3-quicktable-events-lab-ReservationEventBus`를 선택합니다.
24. `ReservationCreatedRule` 규칙을 선택합니다.
25. **Monitoring** 탭을 선택합니다.
26. **Invocations** 메트릭에서 규칙이 트리거된 횟수를 확인합니다.

> [!NOTE]
> Invocations 메트릭이 1 이상이면 EventBridge 규칙이 정상적으로 동작한 것입니다. CloudWatch 메트릭은 1-2분의 지연이 있을 수 있으므로 메트릭이 표시되지 않으면 1-2분 대기 후 페이지를 새로고침합니다.

### 느슨한 결합 확인

27. Lambda 콘솔로 이동합니다.
28. `ReservationProcessor` 함수를 선택합니다.
29. **Code** 탭을 선택합니다.
30. 함수 코드에서 다른 Lambda 함수를 직접 호출하는 코드가 없음을 확인합니다.
...
> - 각 함수는 EventBridge에 이벤트를 발행하기만 하고, EventBridge가 규칙에 따라 적절한 대상 함수를 트리거합니다
>
> **장점**:
>
...
31. DynamoDB 콘솔로 이동합니다.
32. 왼쪽 메뉴에서 **Tables**를 선택합니다.
33. `QuickTableReservations` 테이블을 선택합니다.
34. [[Explore table items]] 버튼을 클릭합니다.
35. `reservationId`가 `res-001`인 항목을 확인합니다.

> [!NOTE]
> DynamoDB 테이블에 예약 데이터가 저장되어 있으면 ReservationProcessor 함수가 정상적으로 동작한 것입니다.
...
restaurant-003은 19:00 시간대에 3개의 예약 가능 슬롯만 있습니다. partySize=4인 예약을 요청하면 TableAvailabilityChecker 함수가 TableUnavailable 이벤트를 발행하고, 태스크 2에서 생성한 EventBridge 규칙이 이 이벤트를 감지하여 NotificationSender 함수를 트리거합니다.
...
>
> 1. 예약 데이터를 DynamoDB에 먼저 저장합니다
> 2. ReservationCreated 이벤트를 EventBridge에 발행합니다
...
3. **Test** 탭을 선택합니다.
4. **Test event action**에서 `Create new event`를 선택합니다.
5. **Event name**에 `TestReservationUnavailableEvent`를 입력합니다.
6. **Event JSON**에 다음 내용을 입력합니다:
...
12. **View Amazon CloudWatch Logs** 링크를 클릭합니다.
13. 최신 로그 스트림을 선택합니다.
14. 로그에서 "Reservation created successfully" 메시지를 확인합니다.

> [!NOTE]
> ReservationProcessor 함수가 정상적으로 실행되었습니다. 이제 DynamoDB에 예약 데이터가 저장되었는지 확인합니다.

15. DynamoDB 콘솔로 이동합니다.
16. 왼쪽 메뉴에서 **Tables**를 선택합니다.
17. `QuickTableReservations` 테이블을 선택합니다.
18. [[Explore table items]] 버튼을 클릭합니다.
19. `reservationId`가 `res-002`인 항목을 확인합니다.

> [!NOTE]
> DynamoDB 테이블에 예약 데이터가 저장되어 있으면 ReservationProcessor 함수가 정상적으로 동작한 것입니다. 예약 불가 시나리오에서도 예약 데이터는 먼저 저장되고, 이후 TableAvailabilityChecker가 비동기적으로 가용성을 확인합니다.

20. AWS Lambda 콘솔로 이동합니다.
21. `TableAvailabilityChecker` 함수를 선택합니다.
22. **Monitor** 탭을 선택합니다.
23. **View Amazon CloudWatch Logs** 링크를 클릭합니다.
24. 최신 로그 스트림을 선택합니다.
25. 로그에서 "Table unavailable: party size (4) exceeds available slots (3)" 메시지를 확인합니다.
26. 로그에서 "TableUnavailable event published" 메시지를 확인합니다.

> [!NOTE]
> TableAvailabilityChecker 함수가 예약 불가를 판단하고 TableUnavailable 이벤트를 발행했습니다.

27. AWS Lambda 콘솔로 이동합니다.
28. `NotificationSender` 함수를 선택합니다.
29. **Monitor** 탭을 선택합니다.
30. **View Amazon CloudWatch Logs** 링크를 클릭합니다.

> [!NOTE]
> Amazon CloudWatch Logs는 1-2분의 지연이 있을 수 있습니다. 로그가 표시되지 않으면 1-2분 대기 후 페이지를 새로고침합니다.

31. 최신 로그 스트림을 선택합니다.
32. 로그에서 "Sending notification for reservation: res-002" 메시지를 확인합니다.
33. 로그에서 "Notification sent" 메시지를 확인합니다.

> [!NOTE]
> NotificationSender 함수가 자동으로 실행되었다면 태스크 2에서 생성한 EventBridge 규칙이 정상적으로 동작하는 것입니다.
>
> 이 실습에서는 SNS 이메일 구독을 설정하지 않으므로 실제 이메일은 수신되지 않습니다. 알림 발송 여부는 NotificationSender 함수의 CloudWatch 로그로 확인합니다.

### EventBridge 규칙 동작 확인

34. EventBridge 콘솔로 이동합니다.
35. 왼쪽 메뉴에서 **Rules**를 선택합니다.
36. **Event bus** 드롭다운에서 `week4-3-quicktable-events-lab-ReservationEventBus`를 선택합니다.
37. `ReservationCreatedRule` 규칙을 선택합니다.
38. **Monitoring** 탭을 선택합니다.
39. **Invocations** 그래프에서 규칙이 실행된 횟수를 확인합니다.
...
40. EventBridge 콘솔로 이동합니다.
41. 왼쪽 메뉴에서 **Rules**를 선택합니다.
42. **Event bus** 드롭다운에서 `week4-3-quicktable-events-lab-ReservationEventBus`를 선택합니다.
43. `TableUnavailableRule` 규칙을 선택합니다.
44. **Monitoring** 탭을 선택합니다.
45. **Invocations** 그래프에서 규칙이 실행된 횟수를 확인합니다.
...
> [!TIP]
> **EventBridge Monitoring 탭 활용**: Monitoring 탭에서는 규칙이 매칭한 이벤트 수, 대상 함수 호출 성공/실패 횟수, 평균 지연 시간 등을 확인할 수 있습니다. 이벤트 기반 아키텍처를 운영할 때 규칙이 정상적으로 동작하는지 모니터링하는 데 유용합니다.

✅ **태스크 완료**: 예약 불가 시나리오가 정상적으로 동작합니다.

다음을 성공적으로 수행했습니다:

- EventBridge 규칙을 생성하여 이벤트 기반 아키텍처를 구축했습니다
...
> [!NOTE]
> EventBridge 규칙 삭제 시 확인 창에 `delete`를 입력해야 합니다. 규칙 이름을 입력하는 것이 아닙니다.
...
- [Lambda와 EventBridge 통합](https://docs.aws.amazon.com/lambda/latest/dg/with-eventbridge.html)

---

## 📚 참고: EventBridge 핵심 개념

### EventBridge 이벤트 버스 (Event Bus)

EventBridge Event Bus는 이벤트를 수신하고 라우팅하는 중앙 허브입니다. 여러 소스에서 발생한 이벤트를 수집하여 규칙에 따라 적절한 대상으로 전달합니다.
...
EventBridge는 이벤트 패턴을 사용하여 특정 이벤트만 필터링하고 대상으로 전달합니다. 이벤트 패턴은 JSON 형식으로 정의되며, 이벤트의 source, detail-type, detail 필드를 기준으로 매칭합니다.
...
EventBridge를 사용하면 서비스 간 직접 의존성을 제거하고 느슨한 결합을 구현할 수 있습니다. 각 AWS Lambda 함수는 다른 함수를 직접 호출하지 않고 이벤트를 발행하기만 하면 됩니다.
...
**느슨한 결합 (EventBridge 방식):**

```
ReservationLambda → EventBridge(ReservationCreated) → TableCheckLambda
                                                               ↓
                                                EventBridge(TableUnavailable)
                                                               ↓
                                                     NotificationLambda
```

- ReservationLambda는 이벤트만 발행
- EventBridge가 이벤트를 적절한 대상으로 라우팅
- TableCheckLambda가 TableUnavailable 이벤트를 발행하면 NotificationLambda가 트리거됨
- 한 서비스 장애 시 다른 서비스는 정상 동작
- 새로운 서비스 추가 시 EventBridge 규칙만 추가
...
### Lambda와 EventBridge 통합

AWS Lambda 함수는 EventBridge와 두 가지 방식으로 통합됩니다.

**1. 이벤트 발행 (Event Publishing):**

AWS Lambda 함수에서 EventBridge에 이벤트를 발행합니다.
...
EventBridge 규칙이 AWS Lambda 함수를 트리거합니다.

```python
def lambda_handler(event, context):
    # EventBridge에서 전달된 이벤트
...
AWS Lambda 함수가 EventBridge에 이벤트를 발행하려면 다음 권한이 필요합니다:
...
EventBridge 규칙이 AWS Lambda 함수를 트리거하려면 다음 권한이 자동으로 추가됩니다:
