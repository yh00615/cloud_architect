...
2. `week5-1-rds-multi-az-lab.yaml` 파일을 확인합니다.
...
7. [[Choose file]] 버튼을 클릭한 후 `week5-1-rds-multi-az-lab.yaml` 파일을 선택합니다.
8. [[Next]] 버튼을 클릭합니다.
...
> [!NOTE]
> 스택 생성에 5-7분이 소요됩니다. **Events** 탭에서 Amazon VPC, 서브넷, NAT Gateway, 보안 그룹, DB 서브넷 그룹이 순차적으로 생성되는 과정을 확인할 수 있습니다.
...
> [!IMPORTANT]
> 이 출력값들은 다음 태스크에서 Amazon RDS 인스턴스를 생성할 때 사용됩니다. 반드시 메모장에 저장하세요.
...
1. AWS Management Console에 로그인한 후 상단 검색창에서 `Amazon RDS`를 검색하고 선택합니다.
2. 왼쪽 메뉴에서 **Databases**를 선택합니다.
3. [[Create database]] 버튼을 클릭합니다.
4. **Engine type**에서 `MySQL`을 선택합니다.
5. **Engine version**에서 콘솔에서 기본 선택되는 MySQL 버전을 사용합니다.
6. **Templates**에서 `Dev/Test`를 선택합니다.

💡 **참고**: Free tier는 Multi-AZ를 지원하지 않으므로 Dev/Test 템플릿을 선택합니다.

### 기본 설정

1. **DB instance identifier**에 `mysql-lab-instance`를 입력합니다.
2. **Username**에 `admin`을 입력합니다.
3. **Master password**에 `MyPassword123!`을 입력합니다.
4. **Confirm password**에 `MyPassword123!`을 입력합니다.
...
2. **Multi-AZ deployment**에서 `Create a standby instance`를 선택합니다.
...
10. 기본(default) 보안 그룹의 **X** 버튼을 클릭하여 제거합니다.
...
1. **Database authentication**에서 `Password authentication`을 선택합니다.
...
7. **Enable auto minor version upgrade**를 체크합니다.
8. **Deletion protection**을 체크 해제합니다.
...
1. Amazon RDS 콘솔에서 생성된 `mysql-lab-instance`를 선택합니다.
2. **Connectivity & security** 탭을 선택합니다.
3. 다음 정보를 확인하고 메모장에 복사합니다:
...
1. Amazon RDS 콘솔에서 `mysql-lab-instance`를 선택합니다.
2. **Actions** > `Reboot`를 선택합니다.
3. **Reboot with failover?**를 체크합니다.
4. [[Confirm]] 버튼을 클릭합니다.
5. 상태가 "Rebooting"으로 변경됩니다.
6. 상태가 "Available"로 변경될 때까지 기다립니다.
...
2. **Connectivity & security** 탭을 선택합니다.
3. **Endpoint**를 확인합니다.
4. **Availability Zone**을 확인합니다.

Endpoint가 동일하게 유지되며, Availability Zone이 태스크 2에서 메모한 AZ와 다른 AZ로 변경되었습니다. 이는 Standby 인스턴스가 Primary로 승격되었음을 의미합니다.

5. **Configuration** 탭을 선택합니다.
6. **Secondary Zone**을 확인합니다.
7. **Logs & events** 탭을 선택합니다.
8. **Events** 섹션에서 "Multi-AZ instance failover started" 및 "Multi-AZ instance failover completed" 이벤트를 확인합니다.
...
2. **Actions** > `Create read replica`를 선택합니다.
3. **DB instance identifier**에 `mysql-lab-replica`를 입력합니다.
...
3. **Snapshot name**에 `mysql-lab-snapshot-manual`을 입력합니다.
4. [[Take snapshot]] 버튼을 클릭합니다.
5. 왼쪽 메뉴에서 **Snapshots**를 선택하여 스냅샷 생성 진행 상황을 확인합니다.
...
3. **Automated backups** 섹션에서 다음을 확인합니다:
...
2. 왼쪽 메뉴에서 **Databases**를 선택합니다.
3. `mysql-lab-replica`를 선택합니다.
4. **Actions** > `Delete`를 선택합니다.
5. `Create final snapshot?`을 체크 해제합니다.
6. `I acknowledge that upon instance deletion, automated backups, including system snapshots and point-in-time recovery, will no longer be available.`를 체크합니다.
7. 확인 창에서 `delete me`를 입력합니다.
8. [[Delete]] 버튼을 클릭합니다.
...
3. `Create final snapshot?`을 체크 해제합니다.
4. `I acknowledge that upon instance deletion, automated backups, including system snapshots and point-in-time recovery, will no longer be available.`를 체크합니다.
