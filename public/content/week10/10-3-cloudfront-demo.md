...
> | Amazon CloudFront | 데이터 전송 (첫 10TB) | ❌  |           $0.126/GB |
> | Amazon CloudFront | HTTP/HTTPS 요청       | ❌  | $0.0120/10,000 요청 |
>
> - **예상 실습 시간**: 30분-1시간
> - **예상 총 비용**: 약 $0.01-0.05 (실습 규모 기준, 실무 환경 온디맨드 기준)
>
> **무료 플랜**
>
...
> **실무 팁**
>
> 💡 CloudFront는 데이터 전송량에 따라 비용이 발생합니다. Price Class를 조정하여 주요 지역만 선택하면 비용을 약 30% 절감할 수 있습니다. 캐시 적중률을 높이면 오리진 요청이 줄어 S3 비용도 절감됩니다.
>
...
22. Amazon CloudFront 콘솔로 이동합니다.
23. 배포를 선택합니다.
24. **Tags** 탭을 선택합니다.
...
> 대기하는 동안 이전 차시 내용을 복습하거나 다음 태스크를 미리 읽어보세요. 페이지를 새로고침하여 최신 상태를 확인할 수 있습니다.

28. 배포 상태가 "Enabled"로 변경될 때까지 기다립니다.
...
1. Amazon CloudFront 콘솔에서 배포 상태가 `Enabled`가 될 때까지 기다립니다.
2. **Distribution domain name**을 복사합니다 (예: `d1234abcd.cloudfront.net`).
3. 새 브라우저 탭을 엽니다.
4. 주소창에 복사한 도메인을 붙여넣고 Enter를 누릅니다.
...
7. **Network** 탭을 선택합니다.
8. **Disable cache** 체크박스를 체크합니다.

> [!IMPORTANT]
> 브라우저 캐시를 비활성화하지 않으면 CloudFront 캐시 헤더를 정확히 확인할 수 없습니다. 브라우저가 로컬 캐시를 사용하여 CloudFront에 요청을 보내지 않을 수 있습니다.

9. 페이지를 새로고침합니다 (Ctrl+R 또는 Cmd+R).
10. `index.html` 요청을 선택합니다.
11. **Headers** 섹션에서 **Response Headers**를 확인합니다.
12. `x-cache` 헤더를 찾습니다.
...
13. 페이지를 다시 새로고침합니다.
14. `x-cache` 헤더가 `Hit from cloudfront`로 변경되었는지 확인합니다.
15. `age` 헤더 값이 증가하는 것을 확인합니다.
16. 주소창에 `http://` + 복사한 도메인을 입력합니다 (예: `http://d1234abcd.cloudfront.net`).
17. 자동으로 `https://`로 리다이렉트되는지 확인합니다.
18. **Network** 탭에서 301 또는 302 리다이렉트 응답을 확인합니다.
...
1. 텍스트 에디터에서 `index.html` 파일을 엽니다.
2. 제목을 "QuickTable v2.0 - 더 빠른 예약 경험"으로 변경합니다.
3. 파일을 저장합니다.
4. Amazon S3 콘솔에서 버킷을 선택합니다.
5. [[Upload]] 버튼을 클릭합니다.
6. 수정한 `index.html` 파일을 선택합니다.

> [!NOTE]
> 동일한 파일명으로 업로드하면 기존 파일을 덮어씁니다. Amazon S3는 자동으로 버전 관리를 하지 않으므로 이전 버전은 삭제됩니다.

7. [[Upload]] 버튼을 클릭합니다.
8. Amazon CloudFront 도메인으로 다시 접속합니다.
9. 여전히 이전 버전이 표시되는 것을 확인합니다 (캐시됨).
...
10. Amazon CloudFront 콘솔에서 배포를 선택합니다.
11. **Invalidations** 탭을 선택합니다.
12. [[Create invalidation]] 버튼을 클릭합니다.
13. **Object paths**에 `/index.html`을 입력합니다.

> [!NOTE]
> 모든 파일을 무효화하려면 `/*`를 입력합니다. 매월 처음 1,000개 경로는 무료이며, 이후 경로당 $0.005가 부과됩니다.

14. Create invalidation 버튼을 클릭합니다.
15. 무효화 상태가 `Completed`가 될 때까지 기다립니다 (1-2분).
16. 브라우저 개발자 도구(F12)에서 **Network** 탭을 선택합니다.
17. **Disable cache** 체크박스가 활성화되어 있는지 확인합니다.
18. 강력 새로고침을 수행합니다 (Ctrl+Shift+R 또는 Cmd+Shift+R).
19. `index.html` 요청을 선택합니다.
20. `x-cache` 헤더가 `Miss from cloudfront`로 표시되는지 확인합니다.

> [!NOTE]
> 무효화 직후 첫 요청은 반드시 `Miss from cloudfront`여야 합니다. 이는 캐시가 제거되어 오리진에서 새 콘텐츠를 가져오는 것을 의미합니다.

21. 변경된 제목 "QuickTable v2.0 - 더 빠른 예약 경험"이 표시되는 것을 확인합니다.
22. 페이지를 다시 새로고침합니다.
23. `x-cache` 헤더가 `Hit from cloudfront`로 변경되는지 확인합니다.
...
3. **Requests** 그래프를 확인합니다 (총 요청 수).
4. **Bytes downloaded** 그래프를 확인합니다 (다운로드된 데이터량).
...
6. **Error rate** 그래프를 확인합니다 (4xx, 5xx 에러 비율).
7. 아래로 스크롤하여 **Popular objects** 섹션을 확인합니다.
8. 가장 많이 요청된 파일을 확인합니다.
...
> [!NOTE]
> CloudFront 배포를 Tag Editor로 찾으려면 **Regions**에서 `us-east-1 (버지니아 북부)`를 선택하거나 `All regions`를 선택하세요. CloudFront는 글로벌 서비스로 us-east-1에 등록됩니다. ap-northeast-2만 선택하면 CloudFront가 표시되지 않습니다.
...
1. Amazon CloudFront 콘솔에서 배포를 선택합니다.
2. [[Disable]] 버튼을 클릭합니다.
3. 확인 창에서 [[Disable]] 버튼을 클릭합니다.
4. 배포 상태가 `Disabled`가 될 때까지 기다립니다.
...
9. 버킷을 선택합니다.
10. [[Empty]] 버튼을 클릭합니다.
11. 확인 창에서 `permanently delete`를 입력합니다.
12. [[Empty]] 버튼을 클릭합니다.
13. 버킷을 다시 선택합니다.
14. [[Delete]] 버튼을 클릭합니다.
15. 확인 창에서 버킷 이름을 입력합니다.
16. [[Delete bucket]] 버튼을 클릭합니다.
