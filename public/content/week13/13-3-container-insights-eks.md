---
title: "Container Insights로 Amazon EKS 모니터링 및 자동 스케일링"
week: 13
session: 3
awsServices:
  - Amazon CloudWatch
  - Amazon EKS
learningObjectives:
  - Container Insights를 활성화하여 Amazon EKS 클러스터의 성능 메트릭과 로그를 수집할 수 있습니다
...
이 실습에서는 Amazon EKS 클러스터에 Container Insights를 활성화하고, CloudWatch를 통해 컨테이너 수준의 성능 메트릭과 로그를 수집 및 분석합니다. Amazon CloudWatch Logs Insights를 사용하여 로그를 쿼리하고, 커스텀 대시보드를 생성하며, 성능 이상을 감지하는 알람을 설정합니다. Horizontal Pod Autoscaler를 통해 자동 스케일링을 구현하여 트래픽 변화에 대응합니다.
...
이 태스크에서는 eksctl을 사용하여 Amazon EKS 클러스터를 생성하고 Container Insights를 위한 Amazon CloudWatch 로깅을 활성화합니다.
...
> CloudWatchAgentServerPolicy를 노드 그룹에 추가하여 Container Insights 메트릭 수집 권한을 부여합니다.
...
## 태스크 2: Container Insights 활성화 (Amazon EKS Add-on 방식)

이 태스크에서는 Amazon EKS Add-on 방식으로 Container Insights를 활성화하고 Amazon CloudWatch 에이전트를 배포합니다.

> [!CONCEPT] Container Insights 아키텍처
> Container Insights는 컨테이너화된 애플리케이션의 성능 메트릭과 로그를 자동으로 수집하는 완전 관리형 모니터링 솔루션입니다.
> 
> **주요 구성 요소**:
...
1. CloudShell에서 Amazon EKS Add-on으로 Container Insights를 활성화합니다:
...
✅ **태스크 완료**: Container Insights가 활성화되었습니다.
...
> [!CONCEPT] Container Insights 메트릭 계층 구조
> Container Insights는 4가지 수준의 메트릭을 제공하여 클러스터부터 개별 컨테이너까지 세밀한 모니터링을 지원합니다.
> 
> **메트릭 수준**:
...
✅ **태스크 완료**: Container Insights 대시보드를 확인했습니다.
...
5. **Add widget** 대화상자가 표시됩니다.
6. **Line** 위젯을 선택합니다.
7. [[Next]] 버튼을 클릭합니다.
8. **Metrics** 탭에서 다음을 선택합니다:
...
9. **Graphed metrics** 탭을 선택합니다.
10. **Statistic**을 `Average`로 설정합니다.
11. **Period**를 `5 minutes`로 설정합니다.
12. [[Create widget]] 버튼을 클릭합니다.

13. [[Add widget]] 버튼을 클릭하여 두 번째 위젯을 추가합니다.
14. **Number** 위젯을 선택합니다.
15. [[Next]] 버튼을 클릭합니다.
16. **ContainerInsights** > **ClusterName**을 선택합니다.
17. `cluster_number_of_running_pods` 메트릭을 선택합니다.
18. [[Create widget]] 버튼을 클릭합니다.

19. [[Add widget]] 버튼을 클릭하여 세 번째 위젯을 추가합니다.
20. **Line** 위젯을 선택합니다.
21. [[Next]] 버튼을 클릭합니다.
22. **ContainerInsights** > **ClusterName, Namespace**를 선택합니다.
23. `namespace_number_of_running_pods` 메트릭을 선택합니다.
24. **default** 네임스페이스를 선택합니다.
25. [[Create widget]] 버튼을 클릭합니다.

26. [[Add widget]] 버튼을 클릭하여 네 번째 위젯을 추가합니다.
27. **Logs table** 위젯을 선택합니다.
28. [[Next]] 버튼을 클릭합니다.
29. **Log groups**에서 `/aws/containerinsights/container-insights-cluster/application`을 선택합니다.
30. 다음 쿼리를 입력합니다:
...
3. [[Select metric]] 버튼을 클릭합니다.
4. **ContainerInsights** 네임스페이스를 선택합니다.
5. **ClusterName** 차원을 선택합니다.
6. `cluster_cpu_utilization` 메트릭을 찾아 체크합니다.
7. [[Select metric]] 버튼을 클릭합니다.
8. **Metric** 섹션에서 다음을 설정합니다:
   - **Statistic**: `Average`
   - **Period**: `5 minutes`

9. **Conditions** 섹션에서 다음을 설정합니다:
...
11. **Notification** 섹션에서 다음을 설정합니다:
...
## 📚 참고: Container Insights 및 Amazon EKS 모니터링

### Container Insights 아키텍처

Container Insights는 컨테이너화된 애플리케이션과 마이크로서비스의 성능 메트릭과 로그를 수집, 집계, 요약하는 완전 관리형 모니터링 솔루션입니다.
