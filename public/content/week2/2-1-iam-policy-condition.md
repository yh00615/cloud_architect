...
이 태스크에서는 CloudFormation을 사용하여 실습에 필요한 IAM 사용자와 Access Key를 자동으로 생성합니다.
...
> [!NOTE]
> 스택 생성에 1-2분이 소요됩니다. **Events** 탭에서 IAM 사용자와 Access Key가 생성되는 과정을 확인할 수 있습니다.
...
- [AWS IAM JSON 정책 요소: Condition](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_elements_condition.html)
- [AWS 전역 조건 컨텍스트 키](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_condition-keys.html)
- [AWS IAM 정책 예제](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_examples.html)
...
**시나리오**: 블랙 프라이데이 이벤트 기간(11월 한 달)만 마케팅 팀에게 프로모션 Lambda 함수 실행 권한 부여
