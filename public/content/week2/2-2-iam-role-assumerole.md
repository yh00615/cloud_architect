...
>   assume-role-policy.json 파일을 참고하여 AWS IAM 사용자가 특정 역할을 맡을 수 있도록 sts:AssumeRole 권한을 부여하는 인라인 정책을 생성하고, Resource 요소로 맡을 수 있는 역할을 명시적으로 지정하여 보안을 강화합니다
...
> [!IMPORTANT]
> 이 출력값들은 태스크 4-6에서 AssumeRole 테스트 시 사용됩니다. 반드시 메모장에 저장하세요.
...
> [!NOTE]
> 콘솔 버전에 따라 "This account (계정ID)" 또는 "This account"로 표시될 수 있습니다. 현재 계정을 선택하는 옵션을 선택합니다.
...
4. [[Next]] 버튼을 클릭합니다.
5. **Name, review, and create** 페이지로 이동합니다.
6. **Role name**에 `S3ReadOnlyRole`을 입력합니다.
7. **Description**에 `Role for read-only access to Amazon S3 buckets`를 입력합니다.
...
10. 아래로 스크롤하여 **Tags - optional** 섹션을 찾습니다.
...
12. [[Create role]] 버튼을 클릭합니다.
13. 역할 생성이 완료되면 **Roles** 페이지로 자동 이동합니다.
14. 화면 상단에 녹색 배너로 "Role S3ReadOnlyRole created"라는 성공 메시지가 표시됩니다.
15. 역할 목록에서 `S3ReadOnlyRole`을 검색하여 생성된 역할을 확인합니다.
...
1. 역할 목록에서 `S3ReadOnlyRole`을 검색합니다.
2. 검색 결과에서 `S3ReadOnlyRole`을 클릭합니다.
3. 역할 상세 페이지가 열립니다.
4. 페이지 상단의 **Summary** 섹션을 확인합니다.
5. **ARN** 필드를 찾습니다.
...
10. 기존 정책 코드를 모두 삭제한 후 다음 정책을 입력합니다:
...
11. JSON 편집기에서 `YOUR_ACCOUNT_ID` 부분을 찾아 실제 계정 ID로 교체합니다.
12. 정책이 올바르게 작성되었는지 확인합니다.
...
13. [[Next]] 버튼을 클릭합니다.
13. **Review and create** 페이지로 이동합니다.
14. **Policy name**에 `AssumeS3ReadOnlyRolePolicy`를 입력합니다.
15. [[Create policy]] 버튼을 클릭합니다.
16. 정책 생성이 완료되면 사용자의 **Permissions** 탭으로 자동 이동합니다.
17. 화면 상단에 녹색 배너로 "Policy AssumeS3ReadOnlyRolePolicy created successfully"라는 성공 메시지가 표시됩니다.
18. **Permissions policies** 섹션에서 `AssumeS3ReadOnlyRolePolicy`가 추가되었는지 확인합니다.
...
1. Amazon S3 버킷 목록을 조회합니다 (읽기 권한 - 성공):
...
1. AWS IAM 콘솔로 이동합니다.
2. 왼쪽 메뉴에서 **Roles**를 선택합니다.
3. 역할 목록에서 `S3ReadOnlyRole`을 검색합니다.
4. `S3ReadOnlyRole` 역할 왼쪽의 라디오 버튼을 선택합니다.

> [!NOTE]
> 역할이 선택되면 라디오 버튼에 점이 표시되고, 상단의 [[Delete]] 버튼이 활성화됩니다.

5. [[Delete]] 버튼을 클릭합니다.
6. 확인 창이 나타나면 입력 필드에 `S3ReadOnlyRole`을 입력합니다.

> [!NOTE]
> 역할 이름을 정확히 입력해야 [[Delete]] 버튼이 활성화됩니다.

7. [[Delete]] 버튼을 클릭합니다.
8. 화면 상단에 녹색 배너로 "Role S3ReadOnlyRole deleted successfully"라는 성공 메시지가 표시됩니다.
9. 역할 목록에서 `S3ReadOnlyRole`이 더 이상 표시되지 않는지 확인합니다.

#### AWS IAM 사용자 인라인 정책 삭제

1. 왼쪽 메뉴에서 **Users**를 선택합니다.
2. 사용자 목록에서 이전에 정책을 추가한 사용자를 검색합니다.
3. 해당 사용자를 클릭합니다.
4. **Permissions** 탭을 선택합니다.
5. **Permissions policies** 섹션에서 `AssumeS3ReadOnlyRolePolicy`를 찾습니다.
6. `AssumeS3ReadOnlyRolePolicy` 왼쪽의 체크박스를 선택합니다.

> [!NOTE]
> 체크박스를 선택하면 체크 표시가 나타나고, 상단의 [[Remove]] 버튼이 활성화됩니다.

7. [[Remove]] 버튼을 클릭합니다.
8. 확인 창이 나타나면 [[Remove]] 버튼을 다시 클릭합니다.
9. 화면 상단에 녹색 배너로 "Policy removed successfully"라는 성공 메시지가 표시됩니다.
10. **Permissions policies** 섹션에서 `AssumeS3ReadOnlyRolePolicy`가 더 이상 표시되지 않는지 확인합니다.

#### lab-user 삭제 (태스크 4에서 생성한 경우만)

> [!NOTE]
> 이 단계는 태스크 4에서 lab-user를 생성한 경우에만 수행합니다. 기존 AWS IAM 사용자를 사용한 경우 이 단계를 건너뜁니다.

1. 왼쪽 메뉴에서 **Users**를 선택합니다.
2. 사용자 목록에서 `lab-user`를 검색합니다.
3. `lab-user`를 클릭합니다.
4. **Security credentials** 탭을 선택합니다.
5. **Access keys** 섹션에서 생성한 Access Key를 찾습니다.
6. Access Key 오른쪽의 **Actions** 드롭다운을 클릭합니다.
7. [[Delete]]를 선택합니다.
8. 확인 창이 나타나면 [[Delete]] 버튼을 클릭합니다.

> [!NOTE]
> Access Key를 먼저 삭제해야 사용자를 삭제할 수 있습니다.

9. 왼쪽 메뉴에서 **Users**를 선택합니다.
10. 사용자 목록에서 `lab-user` 왼쪽의 체크박스를 선택합니다.
11. [[Delete]] 버튼을 클릭합니다.
12. 확인 창이 나타나면 입력 필드에 `lab-user`를 입력합니다.
13. [[Delete]] 버튼을 클릭합니다.
14. 화면 상단에 녹색 배너로 "User lab-user deleted successfully"라는 성공 메시지가 표시됩니다.
