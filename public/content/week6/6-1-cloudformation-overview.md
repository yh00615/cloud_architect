...
1. 다운로드한 `week6-1-cloudformation-demo.zip` 파일의 압축을 해제합니다.
...
1. `s3-with-tags.yaml` 파일을 텍스트 에디터로 엽니다.
2. 템플릿 내용을 확인합니다:
...
3. AWS CloudFormation 콘솔로 이동합니다.
4. `demo-s3-stack`을 선택합니다.
5. [[Update]] 버튼을 클릭합니다.
6. **Prerequisite - Prepare template**에서 `Replace current template`을 선택합니다.
7. **Specify template**에서 `Upload a template file`을 선택합니다.
8. [[Choose file]] 버튼을 클릭한 후 `s3-with-tags.yaml` 파일을 선택합니다.
9. [[Next]] 버튼을 클릭합니다.
10. **Parameters** 페이지에서 기본값을 유지하고 [[Next]] 버튼을 클릭합니다.
11. **Configure stack options** 페이지에서 기본값을 유지하고 [[Next]] 버튼을 클릭합니다.
12. **Review and create** 페이지 하단의 **Change set preview** 섹션으로 스크롤합니다.
...
14. [[Submit]] 버튼을 클릭합니다.
15. 스택 업데이트가 시작됩니다. 상태가 "UPDATE_IN_PROGRESS"로 표시됩니다.

> [!NOTE]
> 스택 업데이트에 1-2분이 소요됩니다. **Events** 탭에서 업데이트 과정을 확인할 수 있습니다.

16. 상태가 "UPDATE_COMPLETE"로 변경될 때까지 기다립니다.
17. **Resources** 탭을 선택합니다.
18. **DemoBucket** 리소스의 **Physical ID**를 클릭합니다.
19. Amazon S3 콘솔에서 **Properties** 탭을 선택합니다.
20. 하단의 **Tags** 섹션으로 스크롤합니다.
21. 템플릿에 정의된 3개의 태그가 있는지 확인합니다:
...
3. **Stack actions** 드롭다운을 클릭합니다.
4. `Detect drift`를 선택합니다.
...
8. 상태가 "DRIFTED"로 표시되는지 확인합니다.

#### 3단계: 드리프트 상세 정보 확인

1. **Stack actions** 드롭다운을 클릭합니다.
2. `View drift results`를 선택합니다.
3. **Resource drift status** 탭에서 드리프트가 발생한 리소스를 확인합니다.
4. **DemoBucket** 리소스의 **Drift status**가 "DRIFTED"인지 확인합니다.
5. **DemoBucket** 리소스를 선택합니다.
6. [[View drift details]] 버튼을 클릭합니다.
7. 드리프트 차이점을 확인합니다:
