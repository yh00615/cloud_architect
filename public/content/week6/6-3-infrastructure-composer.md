...
11. 키 스키마를 다음과 같이 설정합니다:
    - **AttributeName**: `id`
    - **KeyType**: `HASH`
12. **BillingMode**에서 `PAY_PER_REQUEST`를 선택합니다.
...
7. **Handler**에 `index.lambda_handler`를 입력합니다.
8. 오른쪽 패널에서 **Template** 탭을 선택합니다.
9. `ItemsFunction` 리소스의 `Properties` 섹션을 찾습니다.
10. `Handler` 속성 다음에 `Code` 속성을 추가합니다:
...
11. `Code` 속성 다음에 `Environment` 속성을 추가합니다:
...
7. 기존 자동 생성된 `ItemsFunctionRole` 블록 전체를 다음으로 교체합니다:
...
8. `ItemsFunction` 리소스를 찾습니다.
9. `Properties` 섹션에 `Role` 속성을 추가합니다:
...
3. REST API 리소스를 클릭합니다.
4. 오른쪽 속성 패널에서 **Logical ID**를 `ItemsApi`로 변경합니다.
5. **Properties** 섹션을 확장합니다.
6. **Name**에 `Items API`를 입력합니다.
7. 오른쪽 패널에서 **Template** 탭을 선택합니다.
8. `ItemsApi` 리소스 다음에 다음 리소스들을 추가합니다:
...
4. 템플릿의 `Resources` 섹션 끝에 `Outputs` 섹션을 추가합니다 (YAML 들여쓰기 수준 주의: `Resources`와 동일한 레벨):
...
13. [[Next]] 버튼을 클릭합니다.
14. **Stack name**에 `infrastructure-composer-serverless-api`를 입력합니다.
15. [[Next]] 버튼을 클릭합니다.
...
22. AWS CloudFormation 콘솔에서 스택 생성이 시작됩니다.
23. **Events** 탭을 선택합니다.
...
2. **Outputs** 탭을 선택합니다.
3. **Key**가 `ApiUrl`인 출력값을 찾습니다.
4. **Value** 열의 URL을 복사합니다.

> [!NOTE]
> API URL은 다음과 같은 형식입니다: `https://xxxxxxxxxx.execute-api.ap-northeast-2.amazonaws.com/prod/items`

5. CloudShell 아이콘을 클릭합니다.
6. CloudShell이 시작될 때까지 기다립니다.

> [!NOTE]
> CloudShell 시작에 1-2분이 소요될 수 있습니다.

7. 다음 명령어를 실행하여 아이템을 생성합니다:
...
2. `infrastructure-composer-serverless-api` 스택을 선택합니다.
3. [[Delete]] 버튼을 클릭합니다.
4. 확인 대화상자에서 [[Delete]] 버튼을 클릭합니다.
