...
> **프로파일(Profile)이란?**
>
> 프로파일은 조직의 내부 표준이나 규정을 반영한 커스텀 질문 세트입니다. 예를 들어:
>
> - 회사의 보안 정책 준수 여부 확인 질문
> - 특정 규제(HIPAA, PCI-DSS 등) 요구사항 확인 질문
>
> **렌즈 vs 프로파일**:
>
> - **렌즈**: AWS가 제공하는 표준 질문 세트 (Well-Architected Framework, SaaS Lens 등)
> - **프로파일**: 조직이 직접 만드는 질문 세트 (내부 표준, 규제 요구사항)
>
> 이 실습에서는 프로파일을 적용하지 않고 기본 Well-Architected Framework 질문만 사용합니다.
...
> **렌즈(Lens)란?**
>
> 렌즈는 특정 산업이나 기술 영역에 맞춘 질문 세트입니다. AWS Well-Architected Framework 렌즈는 워크로드 생성 시 자동으로 적용됩니다.
>
> **주요 렌즈 종류**:
>
> - **AWS Well-Architected Framework**: 기본 6가지 원칙 (이 실습에서 사용)
> - **SaaS Lens**: SaaS 애플리케이션 설계 모범 사례
> - **Serverless Lens**: 서버리스 아키텍처 모범 사례
> - **Machine Learning Lens**: ML 워크로드 모범 사례
> - **Data Analytics Lens**: 데이터 분석 워크로드 모범 사례
>
> 한 워크로드에 최대 5개의 렌즈를 동시에 적용할 수 있으며, 총 20개까지 적용 가능합니다.
...
> - Application Load Balancer (ALB)로 트래픽 분산
> - Amazon EC2 인스턴스 2대 (프라이빗 서브넷)
> - Amazon RDS MySQL (Multi-AZ 구성)
> - 2개 가용 영역(AZ) 사용
>
> **운영 현황:**
>
...
> | 위험 수준             | 색상        | 의미           | 조치          |
> | --------------------- | ----------- | -------------- | ------------- |
> | **High risk (HRI)**   | 빨간색      | 즉시 개선 필요 | 우선순위 높음 |
> | **Medium risk (MRI)** | 주황색      | 개선 고려 필요 | 우선순위 중간 |
> | **No risk**           | 초록색 체크 | 모범 사례 준수 | 유지 관리     |
>
> 각 숫자는 클릭 가능한 링크로 표시되며, 클릭하면 해당 위험 항목의 상세 내용을 확인할 수 있습니다.
>
> **위험 수준 판단 기준**:
>
> - **High risk**: 선택한 모범 사례가 없거나 매우 적음 (비즈니스에 심각한 영향)
> - **Medium risk**: 일부 모범 사례만 선택됨 (개선 여지 있음)
> - **No risk**: 대부분의 모범 사례를 선택함 (양호한 상태)
...
>
> **Improvement plan 탭 위치**:
>
> - 워크로드 상세 페이지 하단의 탭 메뉴
> - **Overview**, **Lenses**, **Milestones**, **Improvement plan**, **Properties** 탭 중 하나
> - 평가를 완료한 후에만 위험 항목이 표시됩니다
...
> **개선 우선순위 결정 방법**:
>
> 1. **High risk 항목 우선**: 비즈니스에 심각한 영향을 미치는 항목
> 2. **빠른 효과(Quick Wins)**: 적은 노력으로 큰 개선 효과를 얻을 수 있는 항목
> 3. **비용 대비 효과**: ROI가 높은 항목
> 4. **규제 요구사항**: 법적 또는 규제 준수가 필요한 항목
>
> **예시**: 보안 원칙의 "MFA 활성화" 항목은 High risk이면서 빠르게 구현 가능하므로 최우선 순위로 선정합니다.
...
> **마일스톤 활용 방법**:
>
> - **초기 평가**: 현재 상태 기록 (예: "Initial Assessment")
> - **분기별 검토**: 정기적인 개선 진행 상황 추적 (예: "Q1 2025 Review")
> - **주요 변경 전후**: 아키텍처 변경 전후 비교 (예: "Before Migration", "After Migration")
> - **규제 감사**: 규제 준수 증빙 자료로 활용
>
> **마일스톤 비교 기능**:
>
> - 두 마일스톤을 선택하여 위험 수준 변화를 비교할 수 있습니다
> - 개선된 항목과 악화된 항목을 한눈에 파악할 수 있습니다
...
> **보고서 활용 방법**:
>
> - **경영진 보고**: 아키텍처 개선의 필요성 설명 및 예산 확보
> - **팀 공유**: 개발팀과 개선 계획 논의
> - **규제 감사**: 규제 준수 증빙 자료로 활용
> - **정기 검토**: 분기별 또는 반기별 아키텍처 검토 자료
