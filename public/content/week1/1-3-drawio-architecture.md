...
1. 왼쪽 패널의 **네트워킹 및 콘텐츠 전송** 카테고리를 확장합니다.

> [!TIP]
> **AWS 아이콘 검색 방법**: 왼쪽 패널 상단의 검색창에 `RDS`)을 입력하면 아이콘을 빠르게 찾을 수 있습니다. 카테고리를 클릭하면 해당 카테고리의 모든 아이콘이 표시됩니다. 이후 태스크에서도 이 방법을 사용하여 필요한 아이콘을 검색하세요.
