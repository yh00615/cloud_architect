...
1. 왼쪽 패널에서 **instance** 아이콘을 검색합니다. (필요하다면 **그 외 결과** 버튼을 누릅니다.)
...
4. **Security Group** 아이콘을 Web Server 주변에 배치합니다. (같은 보안 그룹에 여러 개를 한 번에 포함 가능)
...
5. 상단 메뉴에서 **파일** > **다음에서 가져오기**를 선택합니다.
6. **PNG...**를 선택합니다.
7. **이미지** 대화상자에서 다음 설정을 확인합니다:
   - **배율**: `100%`로 유지합니다 (기본값).
   - **테두리 두께**: `0`으로 유지합니다.
   - **크기**: `다이어그램`으로 유지합니다.
   - **투명한 배경**: 체크 해제 상태를 유지합니다.
   - **그림자**: 체크 해제 상태를 유지합니다.
   - **격자**: 체크 해제 상태를 유지합니다.
   - **내 다이어그램 복사본 포함**: 체크된 상태를 유지합니다.

> [!NOTE]
> **배율** 설정은 이미지 해상도를 결정합니다. 100%는 기본 해상도이며, 문서나 프레젠테이션에 삽입하기에 적합합니다. 더 높은 해상도가 필요한 경우 200% 또는 300%로 설정할 수 있습니다.

8. [[내보내기]] 버튼을 클릭합니다.
9. **다른 이름으로 저장** 대화상자에서 다음을 설정합니다:
   - **다른 이름으로 저장**: `quicktable-architecture`를 입력합니다 (확장자 `.drawio.png`는 자동으로 추가됩니다).
   - **위치**: `기기`를 선택합니다 (또는 원하는 저장 위치를 선택합니다).
10. [[저장]] 버튼을 클릭합니다.
