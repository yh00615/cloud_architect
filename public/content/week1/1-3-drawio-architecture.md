...
> [!TIP]
> **서브넷 색상 변경**: AWS / Groups 카테고리의 서브넷 아이콘은 예전 버전일 수 있습니다. 색상을 변경하려면 먼저 그룹을 해제해야 합니다.
>
> **그룹 해제 방법**:
> 1. 서브넷을 선택합니다.
> 2. 마우스 오른쪽 버튼 → **Ungroup** 선택 (또는 **⌘ (Cmd) + Shift + U** (Mac) / **Ctrl + Shift + U** (Windows))
> 3. 이제 개별 요소를 선택할 수 있습니다.
>
> **색상 변경**:
> 1. 서브넷의 배경 박스를 선택합니다.
...
>
> **색상 참고**: [AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) 페이지에서 공식 아키텍처 아이콘 PPT를 다운로드하여 최신 색상을 확인할 수 있습니다.
>
> **일반적인 색상 구분**:
>
...
5. 상단 메뉴에서 **파일** > **내보내기** > **PNG**를 선택합니다.
6. **확대/축소** 필드를 `300%`로 설정합니다.

> [!NOTE]
> Draw.io에서는 "해상도" 대신 "확대/축소" 필드로 이미지 크기를 조정합니다. 300% 이상으로 설정하면 문서나 프레젠테이션에 삽입할 때 선명한 품질을 유지할 수 있습니다.

7. [[내보내기]] 버튼을 클릭합니다.
8. 파일명을 `quicktable-architecture.png`로 저장합니다.

> [!TIP]
> 다이어그램을 팀원과 공유하려면 **파일** > **내보내기** > **SVG**를 선택하여 벡터 형식으로도 저장하세요. SVG 파일은 확대해도 선명하며, 웹 문서에 삽입하기 적합합니다.
