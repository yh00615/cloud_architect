...
> [!NOTE]
> 이 실습에서는 보안 그룹 규칙의 설계와 구성 방법을 학습합니다. 실제 트래픽 테스트를 위해서는 각 계층에 Amazon EC2 인스턴스, ALB, Amazon RDS를 배포하고 보안 그룹을 연결해야 합니다. 이는 후속 실습에서 다룹니다.
...
### AWS CloudFormation이 생성한 빈 보안 그룹 확인
...
> [!NOTE]
> 모든 보안 그룹의 인바운드 규칙이 비어 있습니다. AWS CloudFormation이 빈 보안 그룹만 생성했으며, 다음 태스크에서 학생이 직접 인바운드 규칙을 추가합니다.
>
...
4. **Amazon VPC**에서 AWS CloudFormation이 생성한 Amazon VPC를 선택합니다.
...
3. **Amazon VPC**에서 AWS CloudFormation이 생성한 Amazon VPC를 선택합니다.
...
> [!NOTE]
> AWS CloudFormation 스택으로 생성된 Amazon VPC, 서브넷, 보안 그룹 등 모든 리소스가 표시됩니다. 수동으로 생성한 NACL은 태그가 없어 표시되지 않지만, 다음 단계에서 삭제합니다.
...
```bash
# Amazon VPC ID 찾기
Amazon VPC_ID=$(aws ec2 describe-vpcs \
  --region ap-northeast-2 \
  --filters "Name=tag:Name,Values=week3-2-security-group-Amazon VPC" \
  --query 'Vpcs[0].VpcId' \
  --output text)
...
NACL_IDS=$(aws ec2 describe-network-acls \
  --region ap-northeast-2 \
  --filters "Name=vpc-id,Values=$Amazon VPC_ID" "Name=default,Values=false" \
...
for NACL_ID in $NACL_IDS; do
  ...
  for ASSOC_ID in $ASSOCIATIONS; do
    ...
    DEFAULT_NACL=$(aws ec2 describe-network-acls \
      --region ap-northeast-2 \
      --filters "Name=vpc-id,Values=$Amazon VPC_ID" "Name=default,Values=true" \
...
> [!NOTE]
> 스크립트는 week3-2 Amazon VPC의 모든 사용자 정의 NACL을 자동으로 찾아 삭제합니다.
...
- [보안 그룹](https://docs.aws.amazon.com/vpc/latest/userguide/Amazon VPC_SecurityGroups.html)
- [네트워크 ACL](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls.html)
- [보안 그룹과 네트워크 ACL 비교](https://docs.aws.amazon.com/vpc/latest/userguide/Amazon VPC_Security.html)
