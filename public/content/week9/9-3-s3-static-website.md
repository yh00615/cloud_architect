...
1. 다운로드한 `week9-3-quicktable-frontend-lab.zip` 파일의 압축을 해제합니다.
...
> [!NOTE]
> 스택 생성이 시작됩니다. 상태가 "CREATE_IN_PROGRESS"로 표시됩니다.
...
1. 태스크 0에서 압축 해제한 `week9-3-quicktable-frontend-lab` 폴더를 엽니다.
2. 폴더 내에 다음 파일들이 있는지 확인합니다:
   - `index.html`
   - `reservation.html`
   ...
   - `app.js`
   - `buildspec.yml`
   - `cloudformation-template.yaml` (태스크 0에서 사용)
...
10. AWS CloudShell 상단의 **Actions** > `Upload file`을 선택합니다.
11. 파일 선택 창이 열리면 압축 해제한 폴더로 이동합니다.
12. `index.html` 파일을 선택합니다.
...
> `cloudformation-template.yaml` 파일은 태스크 0에서 이미 사용했으므로 CodeCommit에 푸시할 필요가 없습니다.

14. 업로드가 완료되면 같은 방법으로 나머지 파일들을 하나씩 업로드합니다:
    - `reservation.html`
    - `my-reservations.html`
    - `style.css`
    - `app.js`
...
15. 모든 파일 업로드가 완료되면 다음 명령어를 실행하여 파일을 리포지토리 디렉토리로 이동합니다:
...
16. 6개의 파일이 모두 "new file"로 표시되는지 확인합니다.
17. 커밋을 생성합니다:
...
19. 푸시가 성공적으로 완료되었는지 확인합니다.
20. CodeCommit 콘솔로 이동합니다.
21. 생성한 리포지토리를 선택합니다.
...
1. AWS Management Console 상단 검색창에서 `AWS CodePipeline`을 검색하고 선택합니다.
2. 왼쪽 메뉴에서 **Pipelines**를 선택합니다.
3. 태스크 0에서 복사한 `CodePipelineName` 값의 파이프라인을 선택합니다.
...
9. [[Release change]] 버튼을 클릭하여 파이프라인 실행을 시작합니다.
10. 확인 창에서 [[Release]] 버튼을 클릭합니다.
11. Source 단계가 "In progress"로 표시되는 것을 확인합니다.
12. Source 단계가 "Succeeded"로 변경될 때까지 기다립니다.
13. Build 단계가 자동으로 시작되는 것을 확인합니다.
...
14. Build 단계가 "Succeeded"로 변경될 때까지 기다립니다.
...
1. AWS CodePipeline 콘솔에서 파이프라인 실행이 완료될 때까지 기다립니다.

> [!NOTE]
> 전체 파이프라인 실행에 3-5분이 소요됩니다. Source와 Build 단계가 모두 "Succeeded"로 표시되어야 합니다.

2. Amazon S3 콘솔로 이동합니다.
3. 태스크 0에서 복사한 `WebsiteBucketName` 값의 버킷을 선택합니다.
4. **Objects** 탭에서 배포된 파일들을 확인합니다:
   - `index.html`
   - `reservation.html`
   - `my-reservations.html`
   - `style.css`
...
7. QuickTable 메인 페이지가 정상적으로 표시되는지 확인합니다.
...
8. 웹사이트에서 "예약하기" 버튼을 클릭합니다.
9. `reservation.html` 페이지가 정상적으로 표시되는지 확인합니다.
10. "내 예약" 링크를 클릭합니다.
...
1. CloudShell에서 리포지토리 디렉토리로 이동합니다:

```bash
cd ~/quicktable-frontend/<repository-name>
```

2. 변경할 문자열이 있는지 먼저 확인합니다:
...
8. AWS CodePipeline 콘솔로 이동합니다.
9. 태스크 0에서 복사한 `CodePipelineName` 값의 파이프라인을 선택합니다.
10. 파이프라인이 자동으로 실행되는지 확인합니다.
11. Source 단계가 "Succeeded"로 표시될 때까지 기다립니다.
12. Build 단계가 진행되는 것을 확인합니다.
13. Build 단계가 완료될 때까지 기다립니다.
...
2. 태스크 0에서 복사한 `WebsiteBucketName` 값의 버킷을 선택합니다.
3. [[Empty]] 버튼을 클릭합니다.
...
13. 스택 삭제가 완료될 때까지 기다립니다 (2-3분 소요).
