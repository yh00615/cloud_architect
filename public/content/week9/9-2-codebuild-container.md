...
- **AWS IAM 역할**: CodeBuild가 ECR과 CodeCommit에 접근할 수 있는 권한
...
7. [[Choose file]] 버튼을 클릭한 후 `cloudformation-template.yaml` 파일을 선택합니다.
8. [[Next]] 버튼을 클릭합니다.
...
21. 출력값들을 확인하고 메모장에 복사합니다:
    - `ECRRepositoryUri`: ECR 리포지토리 URI (예: `123456789012.dkr.ecr.ap-northeast-2.amazonaws.com/week9-2-codebuild-repo`)
...
이 태스크에서는 간단한 Node.js 애플리케이션과 Dockerfile을 CodeCommit 리포지토리에 푸시합니다. 이 파일들은 다음 태스크에서 CodeBuild가 Docker 이미지를 빌드하는 데 사용됩니다.
...
1. 태스크 0에서 압축 해제한 `week9-2-cicd-lab` 폴더를 엽니다.
2. 폴더 내에 위의 3개 파일이 모두 있는지 확인합니다.
3. AWS Management Console 상단 우측의 AWS CloudShell 아이콘을 클릭합니다.
4. CloudShell이 시작될 때까지 기다립니다.
5. 작업 디렉토리를 생성합니다:
...
6. git-remote-codecommit 헬퍼를 설치합니다:
...
7. 다음 명령어를 실행하여 CodeCommit 리포지토리를 복제합니다:
...
12. `app.js` 파일을 선택합니다.
13. [[Open]] 또는 [[열기]] 버튼을 클릭하여 업로드를 시작합니다.
14. 업로드가 완료되면 같은 방법으로 `package.json` 파일을 업로드합니다.
15. 마지막으로 `Dockerfile` 파일을 업로드합니다.
16. 업로드가 완료되면 다음 명령어를 실행하여 파일을 리포지토리 디렉토리로 이동합니다:
...
18. 3개의 파일과 .git 디렉토리가 모두 표시되는지 확인합니다.
19. 모든 파일을 Git에 추가합니다:
...
21. 3개의 파일이 모두 "new file"로 표시되는지 확인합니다.
22. 커밋을 생성합니다:
...
24. 푸시가 성공적으로 완료되었는지 확인합니다.
25. CodeCommit 콘솔로 이동합니다.
26. 생성한 리포지토리를 선택합니다.
27. **Code** 탭에서 3개의 파일이 모두 표시되는지 확인합니다.

> [!TIP]
> 각 파일을 클릭하여 내용을 확인할 수 있습니다. 특히 `Dockerfile`은 다음 태스크에서 CodeBuild가 이미지를 빌드하는 데 사용하는 중요한 파일입니다.

✅ **태스크 완료**: 애플리케이션 코드가 CodeCommit에 푸시되었습니다.

## 태스크 2: buildspec.yml 분석 및 푸시

이 태스크에서는 CodeBuild가 사용할 빌드 스펙 파일을 분석하고 CodeCommit에 푸시합니다.

### 태스크 설명

buildspec.yml은 CodeBuild가 빌드를 수행하는 방법을 정의하는 파일입니다. 이 파일은 다음 3단계로 빌드 프로세스를 정의합니다:

- **pre_build**: ECR 로그인 및 환경 변수 설정
- **build**: Docker 이미지 빌드 및 태그 지정
- **post_build**: ECR에 이미지 푸시 및 아티팩트 생성

### 상세 단계

1. CloudShell에서 Git 리포지토리 디렉토리에 있는지 확인합니다:
...
3. AWS CloudShell 우측 상단의 **Actions** 드롭다운을 클릭한 후 `Upload file`을 선택합니다.
4. 파일 선택 창이 열리면 압축 해제한 폴더로 이동합니다.
5. `buildspec.yml` 파일을 선택합니다.
6. [[Open]] 또는 [[열기]] 버튼을 클릭하여 업로드를 시작합니다.
7. 업로드가 완료되면 다음 명령어를 실행하여 파일을 리포지토리 디렉토리로 이동합니다:

```bash
mv ~/buildspec.yml .
```

8. 파일이 정상적으로 복사되었는지 확인합니다:
...
13. buildspec.yml이 "new file"로 표시되는지 확인합니다.
14. 커밋을 생성합니다:
...
16. 푸시가 성공적으로 완료되었는지 확인합니다.
17. CodeCommit 콘솔로 이동합니다.
18. 생성한 리포지토리를 선택합니다.
19. **Code** 탭에서 `buildspec.yml` 파일이 표시되는지 확인합니다.
20. `buildspec.yml` 파일을 클릭하여 내용을 확인합니다.
...
1. AWS Management Console 상단 검색창에서 `AWS CodeBuild`를 검색하고 선택합니다.
2. [[Create project]] 버튼을 클릭합니다.
3. **Project name**에 `week9-2-container-build`를 입력합니다.
4. **Description**에 `Build Docker container image`를 입력합니다.
5. **Source** 섹션에서 다음을 설정합니다:
   - **Source provider**에서 `AWS CodeCommit`을 선택합니다.
   - **Repository**에서 생성한 CodeCommit 리포지토리를 선택합니다.
   - **Branch**에서 `main` 또는 `master`를 선택합니다.
6. **Environment** 섹션에서 다음을 설정합니다:
...
7. **Service role**에서 `Existing service role`을 선택합니다.
8. **Role ARN**에 태스크 0에서 복사한 `CodeBuildRoleArn` 값을 입력합니다.

> [!TIP]
> **Allow AWS CodeBuild to modify this service role** 체크박스가 표시되면 체크 해제합니다.
> CloudFormation이 생성한 역할의 정책이 변경되지 않도록 하는 모범 사례입니다.

9. **Buildspec** 섹션에서 `Use a buildspec file`을 선택합니다.
10. **Buildspec name**에 `buildspec.yml`을 입력합니다.
11. **Environment variables** 섹션으로 스크롤합니다.
12. AWS 계정 ID를 확인합니다:
    - AWS Management Console 우측 상단의 계정 이름을 클릭합니다.
    - 드롭다운 메뉴에서 12자리 계정 ID를 확인합니다.
    - 계정 ID를 메모장에 복사합니다.
13. ECR 리포지토리 이름을 추출합니다:
...
14. [[Add environment variable]] 버튼을 클릭하여 첫 번째 환경 변수를 추가합니다:
    - **Name**: `AWS_ACCOUNT_ID`
    - **Value**: 12단계에서 확인한 계정 ID 입력
    - **Type**: `Plaintext`
15. [[Add environment variable]] 버튼을 클릭하여 두 번째 환경 변수를 추가합니다:
    - **Name**: `IMAGE_REPO_NAME`
    - **Value**: 13단계에서 추출한 리포지토리 이름 입력
    - **Type**: `Plaintext`
16. [[Add environment variable]] 버튼을 클릭하여 세 번째 환경 변수를 추가합니다:
    - **Name**: `CONTAINER_NAME`
    - **Value**: `app`
    - **Type**: `Plaintext`
17. 3개의 환경 변수가 모두 올바르게 입력되었는지 확인합니다.

> [!IMPORTANT]
> 환경 변수는 buildspec.yml에서 `$AWS_ACCOUNT_ID`, `$IMAGE_REPO_NAME`, `$CONTAINER_NAME`으로 참조됩니다.
> 값이 정확하지 않으면 빌드가 실패하므로 반드시 확인합니다.

18. **Logs** 섹션에서 `Amazon CloudWatch logs`를 체크합니다.
19. [[Create build project]] 버튼을 클릭합니다.

✅ **태스크 완료**: AWS CodeBuild 프로젝트가 생성되었습니다.

## 태스크 4: 빌드 실행 및 검증

이 태스크에서는 AWS CodeBuild 프로젝트를 실행하여 Docker 이미지를 빌드하고 ECR에 푸시합니다.

### 상세 단계

1. AWS CodeBuild 콘솔에서 생성한 프로젝트를 선택합니다.
2. [[Start build]] 버튼을 클릭합니다.
3. **Start build** 페이지에서 기본값을 유지하고 [[Start build]] 버튼을 클릭합니다.

> [!NOTE]
> 빌드에 3-5분이 소요됩니다. 대기하는 동안 다음 단계에서 빌드 로그를 확인하여 각 단계가 정상적으로 실행되는지 모니터링합니다.

4. **Build logs** 탭을 선택합니다.
5. 빌드 로그가 실시간으로 표시되는지 확인합니다.
...
7. ECR 로그인이 성공했는지 확인합니다 ("Login Succeeded" 메시지).
...
9. Docker 이미지 빌드가 성공했는지 확인합니다 ("Successfully built" 및 "Successfully tagged" 메시지).
...
11. ECR에 이미지가 성공적으로 푸시되었는지 확인합니다 ("digest: sha256:..." 메시지).
...
13. 모든 단계가 "SUCCEEDED" 상태인지 확인합니다.
...
14. 빌드 상태가 "Succeeded"로 변경될 때까지 기다립니다.
15. ECR 콘솔로 이동합니다.
16. 생성한 ECR 리포지토리를 선택합니다.
17. 빌드된 Docker 이미지가 푸시되었는지 확인합니다.
18. 이미지 태그를 확인합니다:
    - `latest` 태그
...
✅ **태스크 완료**: Docker 이미지가 성공적으로 빌드되고 ECR에 푸시되었습니다.
...
- Docker 이미지를 자동으로 빌드하고 ECR에 푸시했습니다
...
> [!IMPORTANT]
> ECR 리포지토리에 이미지가 있으면 CloudFormation 스택 삭제 시 ECR 리포지토리 삭제가 실패합니다.
> **반드시 먼저 이미지를 삭제한 후** CloudFormation 스택을 삭제해야 합니다.

1. Amazon ECR 콘솔로 이동합니다.
2. 생성한 ECR 리포지토리를 선택합니다.
3. 모든 이미지를 선택합니다 (체크박스 클릭).
4. [[Delete]] 버튼을 클릭합니다.
5. 확인 창에서 `delete`를 입력하고 [[Delete]] 버튼을 클릭합니다.

#### AWS CodeBuild 프로젝트 삭제

1. AWS CodeBuild 콘솔로 이동합니다.
2. 생성한 빌드 프로젝트를 선택합니다.
3. **Actions** > `Delete build project`를 선택합니다.
4. 확인 창에서 `delete`를 입력하고 [[Delete]] 버튼을 클릭합니다.

#### AWS CloudFormation 스택 삭제

1. AWS CloudFormation 콘솔로 이동합니다.
2. `week9-2-codebuild-stack` 스택을 선택합니다.
3. [[Delete]] 버튼을 클릭합니다.
4. 확인 창에서 [[Delete]] 버튼을 클릭합니다.
5. 스택 삭제가 완료될 때까지 기다립니다 (2-3분 소요).

> [!NOTE]
> AWS CloudFormation 스택을 삭제하면 ECR 리포지토리, CodeCommit 리포지토리, AWS IAM 역할 등 모든 리소스가 자동으로 삭제됩니다.
...
> 실습 가이드는 9-2 (CodeBuild)와 9-3 (CodePipeline)으로 구성되어 있습니다.
...
> [!NOTE]
> `CONTAINER_NAME`과 `imagedefinitions.json` 파일은 이 실습에서는 직접 사용되지 않지만, Week 9-3 실습(CodePipeline + ECS 배포)에서 필요합니다.
