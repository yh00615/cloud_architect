...
1. **AWS Config role** 섹션에서 `Create AWS Config service-linked role`을 선택합니다.
2. [[Next]] 버튼을 클릭합니다.
3. **AWS Config rules** 페이지에서 규칙을 선택하지 않고 [[Next]] 버튼을 클릭합니다 (규칙은 태스크 3에서 개별적으로 생성).
4. 설정을 검토하고 [[Confirm]] 버튼을 클릭합니다.

> [!NOTE]
> AWS Config 활성화 후 초기 스냅샷 생성에 수 분이 소요되며, SNS 토픽도 이 시점에 생성됩니다.

### 태스크 1.4: SNS 이메일 구독 확인

AWS Config 설정을 완료한 후 Amazon SNS 이메일 구독을 확인합니다.

1. Amazon SNS 콘솔로 이동합니다.
2. 왼쪽 메뉴에서 **Subscriptions**를 선택합니다.
3. Config가 생성한 구독을 찾습니다 (Status: "Pending confirmation").
4. 이메일 받은편지함을 확인합니다.
5. "AWS Notification - Subscription Confirmation" 제목의 이메일을 엽니다.
6. 이메일 본문의 **Confirm subscription** 링크를 클릭합니다.
7. 브라우저에서 "Subscription confirmed!" 메시지를 확인합니다.
8. Amazon SNS 콘솔로 돌아가 페이지를 새로고침합니다.
9. 구독 상태가 "Confirmed"로 변경되었는지 확인합니다.
...
1. AWS Config 콘솔에서 **Resources** 메뉴를 선택합니다.
2. **Resource type** 필터에서 다양한 리소스 타입을 확인합니다.
3. 특정 리소스 타입을 선택합니다 (예: `AWS::S3::Bucket`).
4. 리소스 목록이 표시됩니다.

### 태스크 2.2: 리소스 상세 정보 및 변경 이력 확인

1. 특정 리소스를 클릭하여 상세 정보를 확인합니다.
2. **Resource timeline** 탭에서 변경 이력을 확인합니다 (리소스의 생성 시점, 설정 변경 이력, 변경한 사용자 추적).
3. **Configuration** 탭에서 현재 설정을 확인합니다.
4. **Relationships** 탭에서 관련 리소스를 확인합니다.
5. **Compliance** 탭에서 규정 준수 상태를 확인합니다.
...
> - 리소스 설정 변경 시 자동 평가
> - 규정 위반 리소스 즉시 식별
> - Lambda 함수로 커스텀 규칙 생성 가능
>
> **평가 대상**:
>
> - Amazon S3 버킷 퍼블릭 액세스 차단
> - 암호화 활성화 여부
> - Amazon EC2 인스턴스 Systems Manager 관리
> - AWS IAM 비밀번호 정책
> - 보안 그룹 포트 설정

### 태스크 3.1: Amazon S3 퍼블릭 읽기 금지 규칙 생성

1. AWS Config 콘솔에서 **Rules** 메뉴를 선택합니다.
2. [[Add rule]] 버튼을 클릭합니다.
3. **Select rule type**에서 `Add AWS managed rule`을 선택합니다.
4. 검색창에 `s3-bucket-public-read-prohibited`를 입력합니다.
5. 해당 규칙을 선택하고 [[Next]] 버튼을 클릭합니다.
6. **Name**은 기본값을 유지합니다.
7. **Trigger** 섹션에서:
...
8. **Parameters** 섹션은 기본값을 유지합니다 (파라미터 없음).
9. [[Next]] 버튼을 클릭합니다.
...
11. 설정을 검토하고 [[Add rule]] 버튼을 클릭합니다.
...
1. `s3-bucket-server-side-encryption-enabled` 규칙을 생성합니다:
   - [[Add rule]] 버튼 클릭
   - 규칙 검색 및 선택
...
2. `ec2-instance-managed-by-systems-manager` 규칙을 생성합니다:
   - [[Add rule]] 버튼 클릭
   - 규칙 검색 및 선택
...
> [!NOTE]
> 실습 환경에 EC2 인스턴스가 없는 경우 이 규칙은 "Not applicable" 또는 "Insufficient data"로 표시될 수 있으며, 이는 정상입니다. 이 규칙은 EC2 인스턴스가 AWS Systems Manager에 의해 관리되는지 확인하는 규칙으로, 평가 대상 리소스가 없으면 해당 상태로 표시됩니다.

3. `iam-password-policy` 규칙을 생성합니다:
   - [[Add rule]] 버튼 클릭
   - 규칙 검색 및 선택
...
4. `vpc-sg-open-only-to-authorized-ports` 규칙을 생성합니다:
   - [[Add rule]] 버튼 클릭
   - 규칙 검색 및 선택
   - **Parameters** 섹션에서:
...
3. **Bucket name**에 `config-test-bucket-{계정ID}`를 입력합니다.

> [!NOTE]
> `{계정ID}` 부분은 본인의 AWS 계정 ID로 대체합니다. 예: `config-test-bucket-123456789012` 4. **Region**에서 `Asia Pacific (Seoul) ap-northeast-2`를 선택합니다. 5. **Tags - optional** 섹션에서 [[Add new tag]] 버튼을 클릭한 후 다음 태그를 추가합니다:
...
7. 생성한 버킷을 선택합니다.
8. **Permissions** 탭을 선택합니다.
9. **Block public access (bucket settings)** 섹션에서 [[Edit]] 버튼을 클릭합니다.
10. **Block all public access**를 체크 해제합니다.
11. [[Save changes]] 버튼을 클릭합니다.
12. 확인 창에서 `confirm`을 입력하고 [[Confirm]] 버튼을 클릭합니다.
...
이 태스크에서는 생성한 AWS Config Rules의 평가 결과를 확인하고 규정을 위반한 리소스를 식별합니다. 각 규칙의 준수 상태(Compliant, Noncompliant, Not applicable, Insufficient data)를 검토하고, 위반 리소스의 상세 정보와 위반 시점을 파악합니다. Compliance timeline을 통해 리소스가 언제부터 규정을 위반했는지 추적하고, Configuration 탭에서 현재 설정을 확인하여 어떤 부분이 정책을 위반하는지 분석합니다. 위반 사항을 수정한 후 Config가 자동으로 재평가하여 준수 상태로 변경되는 것을 확인하고, 규정 준수 대시보드에서 전체 준수율과 위반 리소스 통계를 모니터링할 수 있습니다.

### 태스크 5.1: 규정 준수 평가 확인

1. AWS Config 콘솔에서 **Rules** 메뉴를 선택합니다.
2. 생성한 규칙 목록을 확인합니다.
3. 각 규칙의 **Compliance** 상태를 확인합니다.
4. `Noncompliant` 상태인 규칙을 클릭합니다.
5. **Resources in scope** 섹션에서 위반 리소스를 확인합니다.
6. 위반 리소스를 클릭하여 상세 정보를 확인합니다.
7. **Compliance timeline**에서 언제부터 위반되었는지 확인합니다.
8. **Configuration** 탭에서 현재 설정을 확인합니다.

### 태스크 5.2: 위반 리소스 수정

1. `s3-bucket-public-read-prohibited` 규칙을 선택합니다.
2. **Resources in scope** 섹션에서 위반 리소스(버킷 이름)를 확인합니다.
...
3. 새 브라우저 탭을 열고 Amazon S3 콘솔로 이동합니다.
4. 위반 버킷(`config-test-bucket-{계정ID}`)을 선택합니다.
5. **Permissions** 탭을 선택합니다.
6. **Block public access (bucket settings)** 섹션에서 [[Edit]] 버튼을 클릭합니다.
7. **Block all public access**를 체크합니다.
8. [[Save changes]] 버튼을 클릭합니다.
9. 확인 창에서 `confirm`을 입력합니다.
10. [[Confirm]] 버튼을 클릭합니다.

> [!NOTE]
> 퍼블릭 액세스 차단 설정이 적용되었습니다. 이제 AWS Config가 자동으로 재평가합니다.

11. AWS Config 콘솔 탭으로 돌아갑니다.
12. `s3-bucket-public-read-prohibited` 규칙 페이지를 새로고침합니다.
13. [[Re-evaluate]] 버튼을 클릭하여 즉시 재평가를 트리거합니다.
14. 페이지를 새로고침하여 준수 상태 변경을 확인합니다.
15. **Resources in scope** 섹션에서 버킷의 **Compliance** 상태가 "Compliant"로 변경되었는지 확인합니다.
...
1. AWS Config 콘솔에서 **Rules** 메뉴를 선택합니다.
2. 규칙을 선택합니다 (예: `s3-bucket-public-read-prohibited`).
3. **Actions** 드롭다운에서 `Manage remediation`을 선택합니다.
4. **Remediation action** 섹션에서:
   - **Select remediation method**: `Automatic remediation`
   - **Remediation action details**: `AWS-PublishSNSNotification` 선택
5. **Resource ID parameter** 드롭다운에서 아무것도 선택하지 않거나, 기본값을 유지합니다.

> [!WARNING]
> **Resource ID parameter**에 `TopicArn`을 매핑하면 SNS 토픽 ARN 대신 위반 리소스 ID가 들어가게 되어 실행이 실패합니다. Resource ID parameter는 사용하지 않고, Parameters 섹션에서 TopicArn을 직접 입력해야 합니다.
...
> **IAM 역할 설정**:
>
> AWS-PublishSNSNotification SSM Automation 문서를 실행하려면 Config Remediation이 SNS Publish 권한을 가진 IAM 역할을 사용해야 합니다.
>
> **Auto remediation IAM role** 섹션에서:
>
> - 기존 역할이 있으면 SNS Publish 권한이 포함된 역할을 선택합니다
...
7. **Auto remediation**을 활성화합니다.
8. **Retry attempts**: `5`를 입력합니다.
...
> - 위반 리소스 정보를 포함한 동적 메시지가 필요한 경우, 커스텀 Lambda 함수를 사용해야 합니다.
...
4. **Block public access (bucket settings)** 섹션에서 [[Edit]] 버튼을 클릭합니다.
5. **Block all public access**를 체크 해제합니다.
6. [[Save changes]] 버튼을 클릭합니다.
7. 확인 창에서 `confirm`을 입력하고 [[Confirm]] 버튼을 클릭합니다.

> [!NOTE]
> 테스트를 위해 퍼블릭 액세스를 다시 허용합니다. AWS Config가 위반을 감지하면 자동으로 Remediation이 트리거됩니다.

8. AWS Config 콘솔로 이동합니다.
9. **Rules** 메뉴에서 `s3-bucket-public-read-prohibited` 규칙을 선택합니다.
10. 페이지를 새로고침하여 위반 리소스를 확인합니다.
11. **Resources in scope**에서 위반 리소스(`config-test-bucket-{계정ID}`)를 확인합니다.
12. Amazon SNS 이메일 알림이 수신되었는지 확인합니다.

> [!NOTE]
> 퍼블릭 액세스를 다시 해제한 후 Config가 변경을 감지 → 규칙 재평가 → Noncompliant 판정 → Remediation 트리거 → SNS 알림 전송까지의 전체 흐름에 5-10분이 소요될 수 있습니다. 페이지를 여러 번 새로고침하여 상태 변경을 확인합니다.

13. Amazon S3 콘솔로 이동합니다.
14. `config-test-bucket-{계정ID}` 버킷을 선택합니다.
15. **Permissions** 탭을 선택합니다.
16. **Block public access (bucket settings)** 섹션에서 [[Edit]] 버튼을 클릭합니다.
17. **Block all public access**를 체크합니다.
18. [[Save changes]] 버튼을 클릭합니다.
19. 확인 창에서 `confirm`을 입력하고 [[Confirm]] 버튼을 클릭합니다.
...
1. AWS Config 콘솔에서 **Conformance packs** 메뉴를 선택합니다.
2. [[Deploy conformance pack]] 버튼을 클릭합니다.
3. **Select template**에서 샘플 템플릿을 선택합니다:
   - `Operational Best Practices for Amazon S3`
   - 또는 `Operational Best Practices for Security`
4. **Conformance pack name**에 `demo-s3-best-practices`를 입력합니다.
5. **Parameters** 섹션에서 필요한 파라미터를 입력합니다 (선택사항).
6. [[Deploy conformance pack]] 버튼을 클릭합니다.
7. 배포 상태를 확인합니다 (수 분 소요).
8. 배포 완료 후 **Conformance pack details**를 확인합니다.
9. **Rules** 탭에서 포함된 규칙 목록을 확인합니다.
10. **Compliance** 탭에서 전체 준수 상태를 확인합니다.
...
7. 이 실습에서 태그를 추가한 리소스(테스트 버킷, Config Rules 5개)가 표시됩니다.
...
3. `demo-s3-best-practices` Conformance Pack을 선택합니다.
4. [[Delete]] 버튼을 클릭합니다.
...
3. Remediation이 설정된 규칙(`s3-bucket-public-read-prohibited`)을 선택합니다.
4. **Actions** 드롭다운에서 `Manage remediation`을 선택합니다.
5. [[Delete]] 버튼을 클릭하여 Remediation 설정을 제거합니다.
6. 확인 창에서 [[Delete]] 버튼을 클릭합니다.

7. 생성한 규칙을 하나씩 선택합니다.
8. [[Delete rule]] 버튼을 클릭합니다.
...
3. Config가 생성한 토픽을 선택합니다 (이름: `config-topic-{계정ID}`).
4. [[Delete]] 버튼을 클릭합니다.
...
3. 태스크 6.1에서 생성한 Remediation 역할을 선택합니다.
4. [[Delete]] 버튼을 클릭합니다.
...
3. `/aws/config/` 접두사로 시작하는 로그 그룹이 있는지 확인합니다.
4. 로그 그룹이 존재하는 경우, 해당 로그 그룹을 선택합니다.
...
- 메타데이터, 관계, 설정 포함
- 변경 이력 추적의 기본 단위
- Configuration Snapshot은 Delivery Channel 설정에 따라 주기적으로 S3에 전달

> [!NOTE]
> **Configuration Snapshot 생성 방식**:
>
> - AWS Config는 리소스 변경 시 즉시 Configuration Item을 생성합니다.
> - Configuration Snapshot(전체 리소스 스냅샷)은 Delivery Channel의 Frequency 설정에 따라 자동으로 S3에 전달됩니다.
