...
awsServices:
  ...
  - Amazon EventBridge
learningObjectives:
  - Amazon GuardDuty를 활성화하고 위협 탐지를 시작할 수 있습니다
  - Amazon EventBridge 규칙으로 Amazon GuardDuty 결과를 AWS Lambda로 전달할 수 있습니다
  - AWS Lambda 함수로 의심스러운 AWS IAM 사용자를 자동으로 비활성화할 수 있습니다
  - Amazon GuardDuty 콘솔에서 탐지된 위협을 확인할 수 있습니다
prerequisites:
  - AWS 계정 및 관리자 권한
  - 기본적인 보안 개념 이해
  - AWS Lambda 함수 기본 지식
...
> [!DOWNLOAD]
> [week12-3-quicktable-security-demo.zip](/files/week12/week12-3-quicktable-security-demo.zip)
>
...
9. 생성된 토픽의 상세 페이지에서 [[Create subscription]] 버튼을 클릭합니다.
10. **Protocol**에서 `Email`을 선택합니다.
11. **Endpoint**에 알림을 받을 이메일 주소를 입력합니다.
12. [[Create subscription]] 버튼을 클릭합니다.
13. 입력한 이메일 주소로 확인 메일이 발송됩니다.
14. 이메일을 열고 **Confirm subscription** 링크를 클릭합니다.
15. Amazon SNS 콘솔에서 구독 상태가 "Confirmed"로 변경되었는지 확인합니다.
16. 토픽 상세 페이지에서 **ARN**을 복사합니다.
17. 메모장에 저장합니다 (AWS Lambda 함수에서 사용).
...
5. **Use case**에서 `AWS Lambda`를 선택합니다.
6. [[Next]] 버튼을 클릭합니다.
...
13. 생성된 역할을 클릭하여 상세 페이지로 이동합니다.
14. **Permissions** 탭에서 [[Add permissions]] → **Create inline policy**를 클릭합니다.
15. **JSON** 탭을 클릭합니다.
...
11. [[Create function]] 버튼을 클릭합니다.
12. **Code** 탭에서 `lambda_function.py` 파일의 내용을 다음으로 교체합니다:
...
13. **Event type**에서 `Amazon GuardDuty Finding`을 선택합니다.
14. [[Next]] 버튼을 클릭합니다.
...
3. **Sample findings** 섹션으로 스크롤합니다.
4. [[Generate sample findings]] 버튼을 클릭합니다.
5. 샘플 Finding이 생성되었다는 확인 메시지가 표시됩니다.
6. 왼쪽 메뉴에서 **Findings**를 선택합니다.
...
6. **Event JSON** 영역에 다음 테스트 이벤트를 입력합니다:
...
9. **Execution result** 섹션에서 실행 결과를 확인합니다.
...
10. **Log output** 섹션을 확장하여 상세 로그를 확인합니다.
...
11. 이메일 받은편지함을 확인합니다.
12. Amazon GuardDuty 알림 이메일이 수신되었는지 확인합니다.
...
2. 왼쪽 메뉴에서 **Settings**를 선택합니다.
3. **General** 탭을 선택합니다.
4. **Disable Amazon GuardDuty** 섹션으로 스크롤합니다.
5. [[Disable]] 버튼을 클릭합니다.
6. 확인 창에서 비활성화 옵션을 선택합니다.
