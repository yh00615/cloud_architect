...
7. [[Choose file]] 버튼을 클릭한 후 `week11-2-quicktable-datalake-lab.yaml` 파일을 선택합니다.
8. [[Next]] 버튼을 클릭합니다.
...
> 특히 StudentId를 포함한 리소스 이름을 정확히 기록하세요.
...
3. Raw Data 버킷 (`quicktable-raw-{StudentId}-ap-northeast-2`)을 선택합니다.
4. 다음 폴더 구조가 자동으로 생성되었는지 확인합니다:
   - `reservation-data/` - QuickTable 예약 데이터 폴더
   - `restaurant-data/` - QuickTable 레스토랑 정보 폴더
5. `reservation-data/` 폴더를 클릭합니다.
6. `reservations.csv` 파일이 자동으로 업로드되었는지 확인합니다.
...
8. 상위 폴더로 돌아가서 `restaurant-data/` 폴더를 클릭합니다.
9. `restaurants.json` 파일이 자동으로 업로드되었는지 확인합니다.
...
2. 태스크 0에서 생성된 Crawler `quicktable-crawler-{StudentId}`를 확인합니다.
3. Crawler를 선택하여 설정을 확인합니다:
...
5. **Status**가 "Ready" → "Running" → "Stopping" → "Ready"로 변경되는 것을 확인합니다.
6. 실행이 완료되면 **Last run** 정보를 확인합니다.
...
1. 왼쪽 메뉴에서 **Tables**를 선택합니다.
2. **Database** 드롭다운에서 `quicktable_db_{StudentId}`를 선택합니다.
3. 생성된 테이블 `reservation_data`를 클릭합니다.
...
3. **Editor** 탭을 선택합니다.
4. **Database**에서 `quicktable_db_{StudentId}`를 선택합니다.
5. 왼쪽 패널에서 테이블 목록을 확인합니다 (예: `reservation_data`).
6. 첫 번째 쿼리를 실행합니다:
...
3. [[Create crawler]] 버튼을 클릭합니다.
4. **Crawler name**에 `quicktable-restaurants-crawler-{StudentId}`를 입력합니다.
...
14. [[Next]] 버튼을 클릭합니다.
15. **Crawler schedule**에서 `On demand`를 선택합니다.
16. 설정을 검토하고 [[Create crawler]] 버튼을 클릭합니다.

### 태스크 5.2: Crawler 실행 및 테이블 확인

1. 생성된 Crawler `quicktable-restaurants-crawler-{StudentId}`를 선택합니다.
2. [[Run]] 버튼을 클릭합니다.

> [!NOTE]
> Crawler 실행에 1-2분이 소요됩니다. JSON Lines 형식의 파일을 스캔하여 자동으로 테이블을 생성합니다.

3. 실행이 완료되면 왼쪽 메뉴에서 **Tables**를 선택합니다.
4. **Database** 드롭다운에서 `quicktable_db_{StudentId}`를 선택합니다.
5. 새로 생성된 테이블 (예: `restaurant_data`)을 클릭합니다.
...
2. Processed Data 버킷 (`quicktable-processed-{StudentId}-ap-northeast-2`)을 선택합니다.
3. `reservation-analysis-{StudentId}/` 폴더로 이동합니다.
4. 파티션 구조를 확인합니다:
   - `status=confirmed/` 폴더
   - `status=cancelled/` 폴더
5. 각 폴더 내의 Parquet 파일을 확인합니다.
6. 원본 CSV 파일과 Parquet 파일의 크기를 비교합니다.
...
3. [[Empty]] 버튼을 클릭합니다.
4. `permanently delete`를 입력하고 [[Empty]] 버튼을 클릭합니다.
5. `quicktable-processed-{StudentId}-ap-northeast-2` 버킷에 대해 2-4단계를 반복합니다.
6. `quicktable-query-{StudentId}-ap-northeast-2` 버킷에 대해 2-4단계를 반복합니다.
...
1. AWS Glue 콘솔로 이동합니다.
2. 왼쪽 메뉴에서 **Crawlers**를 선택합니다.
3. `quicktable-restaurants-crawler-{StudentId}`를 선택합니다.
4. **Actions** > `Delete`를 선택합니다.
5. 확인 창에서 [[Delete]] 버튼을 클릭합니다.
6. 왼쪽 메뉴에서 **Tables**를 선택합니다.
7. **Database** 드롭다운에서 `quicktable_db_{StudentId}`를 선택합니다.
8. `restaurant_data` 테이블을 선택합니다.
9. **Actions** > `Delete table`을 선택합니다.
10. 확인 창에서 [[Delete]] 버튼을 클릭합니다.
11. `reservation_analysis_{StudentId}` 테이블을 선택합니다.
12. **Actions** > `Delete table`을 선택합니다.
...
2. `week11-2-quicktable-datalake-stack` 스택을 선택합니다.
3. [[Delete]] 버튼을 클릭합니다.
4. 확인 창에서 [[Delete]] 버튼을 클릭합니다.
5. 스택 삭제가 완료될 때까지 기다립니다 (3-5분 소요).
...
2. `quicktable-raw-{StudentId}-ap-northeast-2` 버킷을 선택합니다.
3. [[Delete]] 버튼을 클릭합니다.
4. 버킷 이름을 입력하고 [[Delete bucket]] 버튼을 클릭합니다.
5. `quicktable-processed-{StudentId}-ap-northeast-2` 버킷에 대해 2-4단계를 반복합니다.
6. `quicktable-query-{StudentId}-ap-northeast-2` 버킷에 대해 2-4단계를 반복합니다.
